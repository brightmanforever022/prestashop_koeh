...
<tr class="product-line-row" id="orderDetailRow_{$product['id_order_detail']}" data-prod_spl_ref="{$product.product_supplier_reference}">
	...
	<td>
		<a class="{if ($product.product_quantity_return > 0) || ($product.product_quantity == 0)}text-red{/if}" href="{$link->getAdminLink('AdminProducts')|escape:'html':'UTF-8'}&amp;id_product={$product['product_id']|intval}&amp;updateproduct&amp;token={getAdminToken tab='AdminProducts'}">
        ...
        <td>
            {if isset($product.avForCur)}
                {l s="avail for ship:"}
                {$product.avForCur}
                <br/>
                {if $product.avForCur>0}
                    {if $product.avForCur>$product.dbkShopsQty}
                    ...
                    {/if}
                {else}
                    <span class="list-action-enable action-disabled stockStatusExpDelivery" rel="#expDelivery{$product['id_order_detail']}"><i class="icon-remove"></i></span>
                    
                    <div class="bootstrap hidden" id="expDelivery{$product['id_order_detail']}">
                    {if !empty($product.expDelivery)}
                    <table class="table" style="width:100%">
                        <tr>
                            <th>{l s='Sup Order id'}</th>
                            <th>{l s='Exp. Date'}</th>
                        </tr>
                        {foreach from=$product.expDelivery item=so}
                        <tr>
                            <td>{$so['id']} / {$so['order_name']}</td>
                            <td>{dateFormat date=$so['exp_arrive_date'] full=false}</td>
                        </tr>
                        {/foreach}
                    </table>
                    {else}
                        <p>&nbsp;{l s='No delivery expected'}&nbsp;</p>
                    {/if}
                    </div>
            ...
            {l s='No stock status info available'}
            {/if}
        </td>
        <td class="text-center shippedCol">
            ...
            {/if}
            {*if ($product.product_quantity-$product.product_quantity_refunded-$product.product_quantity_return-$product.shipped>0) *}
            {*if $product.shipped}action-enabled{else}action-disabled{/if*}
            <span class="list-action-enable {if ($product.product_quantity-$product.product_quantity_refunded-$product.product_quantity_return-$product.shipped<=0  && $product.shipped>0)}action-enabled{else}action-disabled{/if}" >{if ($product.product_quantity-$product.product_quantity_refunded-$product.product_quantity_return-$product.shipped<=0 && $product.shipped>0)}<i class="icon-check"></i>{else}<i class="icon-remove"></i>{/if}</span>
            <br/>
            <span class="shippedNum">{$product.shipped}</span>/<span class="needToBeShippedNum">{$product.product_quantity-$product.product_quantity_refunded-$product.product_quantity_return}</span>
            {if !empty($product.who_shipped)}
            </span>
            {/if}
        </td>
	...
	<td class="productQuantity text-center">
		...
		<span class="product_quantity_edit" style="display:none;">
			<input type="text" name="product_quantity" class="edit_product_quantity" value="{$product['product_quantity']|htmlentities}" autocomplete="off" />
		</span>
		{/if}
	</td>
	...
	{if $order->hasBeenDelivered() || $order->hasProductReturned()}
		<td class="productQuantity text-center">
			<span class="{if $product['product_quantity_return']>0}label label-danger{/if}">{$product['product_quantity_return']}</span>
	...
	{if $stock_management}<td class="text-center product_stock_physical" rel="" data-id="{$product.product_supplier_reference}">{$product['physical_stock']}</td>{/if}
	...
	{if ( Configuration::get('PS_ORDER_RETURN') && ($order->hasBeenDelivered() || $product.shipped>0) && ($product['product_quantity_return'] + $product['product_quantity_refunded'] < $product['product_quantity']) )}
	...
	{elseif ( Configuration::get('PS_ORDER_RETURN') && ($order->hasBeenDelivered() || $product.shipped>0) )}
	...
	<td class="productDetailNote">
		{if !empty($product['note']) && !empty($product['note_employee_n_date'])}
		<span data-toggle="tooltip" class="label-tooltip" data-original-title="{$product['note_employee_n_date']}" data-html="true" data-placement="top">
		{/if}

		{if empty($product['note'])}--{else}{$product['note']}{/if}

		{if !empty($product['note']) && !empty($product['note_employee_n_date'])}</span>{/if}

	</td>
	<td class="productDetailNotePrivate">
		{if !empty($product['note_private']) && !empty($product['note_employee_n_date'])}
		<span data-toggle="tooltip" class="label-tooltip" data-original-title="{$product['note_employee_n_date']}" data-html="true" data-placement="top">
		{/if}

		{if empty($product['note_private'])}--{else}{$product['note_private']}{/if}
	
		{if !empty($product['note_private']) && !empty($product['note_employee_n_date'])}</span>{/if}
	</td>
	
	{if ($can_edit && !$order->hasBeenDelivered() && $product.shipped<$product.product_quantity)}
