...
*}
{if !isset($orderDocumentsOnlyTable) && $can_edit}
			...
			<tr>
				...
				{*<th>
					...
					<span class="title_box ">{l s='Paid'}</span>
                                ...
                                <th>
					<span class="title_box ">{l s='Shipped only'}</span>
				</th>
                                <th>
			...
			{foreach from=$order->getDocuments() item=document}
				...
				{/if}
                        ...
                        <td class="documentPaid">
                        	<a href="#" 
                        		class="{if $can_edit}documentPaidChangeLink{/if} list-action-enable {if $document->paid}action-enabled{else}action-disabled{/if} label-tooltip"
                        		data-toggle="tooltip"
                        		{if $document->paid}
                        		title="At {dateFormat date=$document->payment_date} by {$document->paid_employee_name}"
                        		{/if}
	                      		>
	                      		{if $document->paid}
                        ...
                        <td class="documentPaid">
                        	<a href="#" 
                        		class="list-action-enable {if $document->shipped_products_only}action-enabled{else}action-disabled{/if} label-tooltip"
                        		data-toggle="tooltip">
	                      		{if $document->shipped_products_only}
	                      		<i class="icon-check"></i>
	                      		{else}
	                      		<i class="icon-remove"></i>
	                      		{/if}
			...
			{foreachelse}
				<tr>
					<td colspan="5" class="list-empty">
						...
						{if isset($invoice_management_active) && $invoice_management_active && $can_edit}
...
{if !isset($orderDocumentsOnlyTable) && $can_edit}
        ...
        <form method="post" id="addInoiceForm" action="{$current_index}&amp;vieworder&amp;submitAddInvoice&amp;id_order={$order->id}{if isset($smarty.get.token)}&amp;token={$smarty.get.token|escape:'html':'UTF-8'}{/if}">
            ...
            <button type="button" id="submitAddInvoice" class="btn btn-primary pull-right" name="submitAddInvoice" >{l s='Add'}</button>
            <div style="overflow:auto">
                <div class="pull-right">
                <a href="#" class="btn btn-primary" id="cancelOrderPackageBtn">{l s='Cancel picking list'}</a>&nbsp;
                <a href="#" class="btn btn-primary" id="showOrderPackageBtn">{l s='Show picking list'}</a>&nbsp;&nbsp;
                </div>
            ...
            <select name="shipped_products_only" id="addInvoiceShippedProductsOnly" autocomplete="off" style="width:auto; float:left;">
                <option value="1">{l s='Shipped products only'}</option>
                <option value="0">{l s='All products'}</option>
            </select>
            <select name="invoice_template_id" id="addInvoiceInvoiceTemplateSel" style="width:auto; float:left;" autocomplete="off">
                {foreach from=$invoiceTemplates item='invoiceCategory'}
                    ...
                    {if $template.id|in_array:$suggestedInvoiceTemplates}
                    {$optionStyle='color:#00CC00'}
                    {/if}
                        <option style="{$optionStyle}" value="{$template.id}" data-require_vat="{$template.require_vat}"
                          >{$template.name} ({$template.id})</option>
                    {/foreach}
                    </optgroup>
                {/foreach}
            </select>&nbsp;
            ...
            <select name="id_order_package" id="addInvoiceOrderPackageId">
            	<option value="0">{l s='No picking list, products selected manualy'}</option>
            {foreach $order_packages as $order_package}
            	<option value="{$order_package['id_order_package']}"
            	   >By {$order_package['employee_name']} at {dateFormat date=$order_package['date_add'] full=true}{if $order_package['cancelled']} ({l s='cancelled'}){/if}</option>
            {/foreach}
            </select>
       ...
       <h4>{l s='Invoice text. Will be put in [orderInvoiceTxt] variable in invoice template'}</h4>
        ...
        <form method="post" id="cancelOrderPackageForm" action="{$current_index}&amp;viewOrder&amp;submitCancelOrderPackage&amp;id_order={$order->id}{if isset($smarty.get.token)}&amp;token={$smarty.get.token|escape:'html':'UTF-8'}{/if}">
...
//<![CDATA[
    ...
    var cancelOrderPackageText = '{l s='Should shipped marker be removed and stock corrected?' js='1'}';
//]]>
</script>
{/if}