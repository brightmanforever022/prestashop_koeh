	...
	function addProductProcess()
	{
		if (customization_errors) {
			...
			if(productDataToAdd.cstmr_main_excluded == true){
				...
				$.ajax({
					type:"POST",
					url: "{$link->getAdminLink('AdminMainexclCustomers')|addslashes}",
					dataType: "json",
					data : {
						ajax: "1",
						token: "{getAdminToken tab='AdminMainexclCustomers'}",
			...
			<div class="col-lg-9">
				<select id="id_order_type" name="id_order_type">
					{foreach from=$order_types item='order_type'}
						<option value="{$order_type.id_order_type}">{$order_type.name}</option>
					{/foreach}
				</select>
			</div>
		</div>
			...
			<div class="row">
				<div class="col-lg-1">
					<div class="data-focus">
						<span>{l s='Total items'}</span><br/>
						<span id="total_items" class="size_l text-success"></span>
					</div>
				</div>
				<div class="col-lg-1">
					<div class="data-focus">
						<span>{l s='Total quantity'}</span><br/>
