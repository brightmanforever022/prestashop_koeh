...
{if !$simple_header}
	<script type="text/javascript">
		$(function() {
			...
			$('table.table.{$table}').on('click', '#checkSwitchAll', function(){
                if($(this).prop('checked')){
                    checkDelBoxes($('#form-{$list_id}').get(0), '{$list_id}Box[]', true);
                }
                else{
                    checkDelBoxes($('#form-{$list_id}').get(0), '{$list_id}Box[]', false);
                }
            });
...
{/if}
    ...
    {foreach $fields_display AS $key => $params}
        {if ($params.type == 'select') && isset($params.multiple) && $params.multiple}
        <div id="{$params.filter_key|replace:'!':''}MultySelectCont" ></div>
        {/if}
    {/foreach}
											...
											{html_options options=$params.list selected=$params.value}
				                                                ...
				                                                //<![CDATA[
                                                                                $("#{$params.filter_key|replace:'!':''}MultySelect").multipleSelect({
                                                                                    multiple: true,
                                                                                    {if isset($params.multipleWidth)}
                                                                                    multipleWidth: {$params.multipleWidth},
                                                                                    onOpen: function(){
                                                                                    	$('#{$params.filter_key|replace:'!':''}MultySelectCont .ms-drop').width('{$params.multipleWidth+30}px');
                                                                                    },
                                                                                    {/if}
                                                                                    dropWidth:250,
                                                                                    container: '#{$params.filter_key|replace:'!':''}MultySelectCont',
                                                                                });
                                                                                //]]>
                                                                                </script>
										{else}
