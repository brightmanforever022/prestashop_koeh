        ...
        foreach ($filters as $key => $value) {
            ...
            if ($value != null && !strncmp($key, $prefix.$this->list_id.'Filter_', 7 + Tools::strlen($prefix.$this->list_id))) {
                ...
                if ($field = $this->filterToField($key, $filter)) {
                    ...
                    if (($type == 'date' || $type == 'datetime' || $type == 'multiint') && is_string($value)) {
                    ...
                    if (is_array($value)) {
                        if( $type == 'multiint' ){
                            ...
                            $value = array_filter($value, function($val){ return is_numeric($val); });
                            $value = array_map(function($val){ return  intval($val); }, $value);

                            $sql_filter .= ' AND '. ($check_key ?  $alias.'.' : '').pSQL($key) .
                                ' IN('. implode(',', $value) .')';
                        }
                        else{
