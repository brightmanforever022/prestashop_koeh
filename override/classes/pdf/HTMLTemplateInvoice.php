    ...
    */
    private function replaceToken($html)
    {
        ...
        */
        ...
        // Insert QR code of the order ID
        $qrCodeImageName = uniqid() .'.png';
        $qrCodeImagePath = _PS_TMP_IMG_DIR_ . $qrCodeImageName ;
        $qrCodeImageUrl = _PS_TMP_IMG_ . $qrCodeImageName;
        QRcode::png($this->order->id, $qrCodeImagePath, QR_ECLEVEL_H, 4, 4);
        $html = str_replace('[qrcode_order_id]', '<img src="'. $qrCodeImageUrl .'" width="25"/>', $html);
        
        // Customer's discount
        $html = str_replace('[customerDiscount]', number_format( $customer->discount, 2), $html);
