...
{
    /**
     * Overrided how stock updated when order is "payment error/cancelled"
     *
                ...
                if (Validate::isLoadedObject($old_os)) {
                    ...
                    if ($new_os->logable && !$old_os->logable) {
                        ...
                        if (!Pack::isPack($product['product_id']) &&
                            ...
                            !StockAvailable::dependsOnStock($product['id_product'], (int)$order->id_shop)) {
                                StockAvailable::updateQuantity($product['product_id'], $product['product_attribute_id'], 
                                    -intval($product['product_quantity'] - $product['product_quantity_refunded'] - $product['product_quantity_return']),
                                    $order->id_shop);

                            // report stock update
                            $msss_client->scheduleStockUpdateById(
                                $product['product_id'],
                    ...
                    elseif (!$new_os->logable && $old_os->logable) {
                        ...
                        if (!Pack::isPack($product['product_id']) &&
                            ...
                            // @overrided quantity to re-stock
                            StockAvailable::updateQuantity($product['product_id'], $product['product_attribute_id'],
                                intval($product['product_quantity'] - $product['product_quantity_refunded'] - $product['product_quantity_return']), $order->id_shop);
