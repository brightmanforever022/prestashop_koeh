        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            foreach ($package_list as $id_address => $packageByAddress) {
                foreach ($packageByAddress as $id_package => $package) {
                    ...
                    // rename product, if it is actual
                    if( isset($extra_vars['custom_product_name']) ){
                        foreach($package['product_list'] as $pi => $productInfo){
                            foreach( $extra_vars['custom_product_name'] as $customProductInfo ){
                                if( ($productInfo['id_product'] == $customProductInfo['id_product'])
                                    && ($productInfo['id_product_attribute'] == $customProductInfo['id_product_attribute'])
                                ){
                                    $package['product_list'][$pi]['name'] = $customProductInfo['name'];
                                }
                            }
                        }
                    }
                    ...
                    // set order type
                    if( !empty($extra_vars['id_order_type']) ){
                        $order->id_order_type = intval($extra_vars['id_order_type']);
                    }
                    else{
                        $orderTypeDefaultId = Db::getInstance()->getValue('
                            SELECT id_order_type
                            FROM `'._DB_PREFIX_.'order_type`
                            WHERE `default` = 1
                        ');
                        if( $orderTypeDefaultId !== false ){
                            $order->id_order_type = intval($orderTypeDefaultId);
                        }
                    }
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (!$order_creation_failed && isset($order->id)) {
                    ...
                    foreach ($order->product_list as $product) {
                        ...
                        $product_price = Product::getTaxCalculationMethod($order->id_customer) == PS_TAX_EXC ? Tools::ps_round($price, 2) : $price_wt;
                    ...
                    if ($old_message) {
                        ...
                        if (!$customer_message->add()) {
                            ...
                            $customerOrderComment = nl2br($update_message->message);
                            /**
                             * Notify admin of a comment in new order
                             */
                            $adminEmail = 'info@koehlert.com';
                            $commentNotifySubject = 'Kundennachricht zur Bestellung';
                            $commentNotifySubject .= ' #'. $order->id;
                            $commentNotifyTo = new Swift_RecipientList();
                            $commentNotifyTo->addTo($adminEmail);
                            
                            $commentNotifyConnection = new Swift_Connection_NativeMail();
                            $commentNotifyConnectionSwift = new Swift($commentNotifyConnection, 'www.koehlert.com');
                            $commentNotifyConnectionSwiftMessage = new Swift_Message($commentNotifySubject);
                            $commentNotifyConnectionSwiftMessage->setCharset('utf-8');
                            $commentNotifyConnectionSwiftMessage->attach(new Swift_Message_Part($customerOrderComment, 'text/html', '8bit', 'utf-8'));
                            $commentNotifyConnectionSwiftMessage->headers->setEncoding('Q');
                            $commentNotifyConnectionSwiftMessage->setReplyTo($this->context->customer->email);
                            
                            try{
                                $commentNotifyConnectionSwift->send($commentNotifyConnectionSwiftMessage, $commentNotifyTo, new Swift_Address($adminEmail, $adminEmail));
                            }
                            catch( Exception $e ){
                                PrestaShopLogger::addLog('Failed to notify admin on order comment', 3);
                            }
                            
                            $commentNotifyConnectionSwift->disconnect();
