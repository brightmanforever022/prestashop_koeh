    ...
    protected function assignAttributesGroups()
    {
        ...
        if (is_array($attributes_groups) && $attributes_groups) {
            ...
            foreach ($attributes_groups as $k => $row) {
                ...
                // should filled with date when available quantity supplied 
                $combinations[ $row['id_product_attribute'] ]['expected_delivery'] = false;
                // dates when supplies expected
                $combinations[ $row['id_product_attribute'] ]['expected_deliveries'] = array();
                foreach( $combinations_expected_delivery as $exp_del ){
                    if( $exp_del['id_product_attribute'] == $row['id_product_attribute'] ){
                        $combinations[ $row['id_product_attribute'] ]['expected_deliveries'][] = 
                            array(
                ...
                if( count($combinations[ $row['id_product_attribute'] ]['expected_deliveries']) ){
                    $quantityAvailableToSupplyDate = $combinations[$row['id_product_attribute']]['quantity'];
                    foreach( $combinations[ $row['id_product_attribute'] ]['expected_deliveries'] as $expected_delivery_day ){
                        $quantityAvailableToSupplyDate += $expected_delivery_day['quantity'];
                        if( $quantityAvailableToSupplyDate > 0 ){
                            $combinations[ $row['id_product_attribute'] ]['expected_delivery'] = $expected_delivery_day['date'];
                            break;
                        }
                    }
                }
            ...
            $dbkStock = $this->product->getDbkStock();

            foreach( $combinations as $id_product_attribute => $combination ){
                foreach($groups as $gri => $group){
                    if( in_array($combination['attributes'][0], array_keys($group['attributes'])) ){
                        ...
                        if( isset($dbkStock[$id_product_attribute]) ){
                            $groups[$gri]['dbk_quantity'][ $combination['attributes'][0] ] = $dbkStock[$id_product_attribute];
                        }
                        else{
                            $groups[$gri]['dbk_quantity'][ $combination['attributes'][0] ] = 0;
                        }
                    }
                }
            }
