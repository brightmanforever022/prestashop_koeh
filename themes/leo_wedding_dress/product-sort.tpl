...
<form id="productsSortForm{if isset($paginationId)}_{$paginationId}{/if}" action="{$request|escape:'html':'UTF-8'}" class="productsSortForm form-horizontal pull-left hidden-xs">
    <div class="select">
        <label for="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}" style="margin-bottom: 0;height: 30px;">{l s='Sort by'}:</label>
        <select id="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}" class="selectProductSort form-control" autocomplete="off" >
            ...
            {if $page_name=='category'}
                <option value="top_seller:asc"{if $orderby eq 'top_seller'} selected="selected"{/if}>{l s='Favorites'}</option>
                <option value="best_seller:desc"{if $orderby eq 'best_seller'} selected="selected"{/if}>{l s='Bestseller'}</option>
                <option value="date_add:desc"{if $orderby eq 'date_add'} selected="selected"{/if}>{l s='New'}</option>
            {else}
                {if !$PS_CATALOG_MODE}
