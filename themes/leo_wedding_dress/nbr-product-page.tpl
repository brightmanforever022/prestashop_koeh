...
*}
{*if isset($showPager) || isset($p) AND $p*}
	...
	{/if}
	<!-- nbr product/page -->
	{*if isset($showPager) || $nb_products > $nArray[0]*}
		<form action="{if !is_array($requestNb)}{$requestNb|escape:'html':'UTF-8'}{else}{$requestNb.requestUrl|escape:'html':'UTF-8'}{/if}" method="get" class="nbrItemPage form-horizontal">
			<div class="clearfix selector1 form-group">
				...
				{if is_array($requestNb)}
					...
					{/foreach}
				{/if}
				<label class="col-xs-6 col-md-7 control-label"><span>{l s='Show '} {l s='per page'}</span></label>
