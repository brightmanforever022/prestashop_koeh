		...
		<table id="order-list" class="table table-bordered footab">
			<thead>
				<tr>
					...
					<th data-sort-ignore="true" class="item">{l s='Photos'}</th>
			...
			<tbody>
				{foreach from=$orders item=order name=myLoop}
					<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
						...
						<td class="">
							<a class="btn btn-outline button button-small btn-sm" href="{$base_dir}modules/khlbasic/admin.php?action=get_order_images&amp;id_order={$order.id_order}">{l s='Photos download'}</a>
							<a class="btn btn-outline button button-small btn-sm" href="{$base_dir}modules/khlbasic/admin.php?action=get_order_eans&amp;id_order={$order.id_order}">{l s='EANs download'}</a>
						</td>
