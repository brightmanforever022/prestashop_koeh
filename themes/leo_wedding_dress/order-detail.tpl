...
<div class="box box-small clearfix">
	...
	{/if}
	<p class="dark">
		<strong>{l s='Order ID %s - placed on' sprintf=$order->id} {dateFormat date=$order->date_add full=0}</strong>
	</p>
</div>
...
{if count($order_invoices)}
<div>
	<h1 class="page-heading">{l s='Order invoices'}</h1>
	<div class="table_block table-responsive">
		<table class="table table-bordered">
		<thead>
			<tr>
				<th class="first_item">{l s='Date'}</th>
				<th class="">{l s='Amount'}</th>
				<th class="">{l s='Payment status'}</th>
				<th class="last_item">{l s='Invoice number'}</th>
			</tr>
		</thead>
		<tbody>
		{foreach $order_invoices as $invoice}
			<tr class="">
				<td class="">{dateFormat date=$invoice.date_add full=0}</td>
				<td>{displayWtPriceWithCurrency price=$invoice.sum_to_pay currency=$currency}</td>
				<td>
					{if $invoice.paid}{l s='Paid'}{else}{l s='Not paid'}{/if}
				</td>
				<td><a href="{$invoice.invoice_pdf_link}">{$invoice.number_formatted}</a></td>
			</tr>
		{/foreach}
		</tbody>
	</table>
	...
	<table class="table table-bordered">
		<thead>
			<tr>
				...
				<th class="{if $return_allowed}item{else}first_item{/if}">{l s='Image'}</th>
				<th class="item">{l s='Product'}</th>
				<th class="item">{l s='Shipped'}</th>
				...
				{if isset($product.customizedDatas)}
					<tr class="item">
						...
						<td>
							...
							<a href="{$link->getProductLink($product.product_id)}">
							<img src="{$link->getImageLink($product.link_rewrite, $product.image->id, 'small_default')|escape:'html':'UTF-8'}" />
							</a>
						</td>
						<td class="bold">
							<a href="{$link->getProductLink($product.product_id)}">{$product.product_name|escape:'html':'UTF-8'}</a>
						</td>
						<td>{if $product.shipped == '1'}{l s='Yes'}{else}{l s='No'}{/if}</td>
						<td>
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
					<tr class="item">
						...
						<td class="text-center">
							{*<label for="cb_{$product.id_order_detail|intval}">{if $product.product_reference}{$product.product_reference|escape:'html':'UTF-8'}{else}--{/if}</label>*}
							<a href="{$link->getProductLink($product.product_id)}"  target="_blank">
							<img src="{$link->getImageLink($product.link_rewrite, $product.image->id, 'small_default')|escape:'html':'UTF-8'}" />
							</a>
						</td>
						<td class="bold">
...
{if false && $carriers|count > 0 && isset($carriers.0.carrier_name) && $carriers.0.carrier_name}
