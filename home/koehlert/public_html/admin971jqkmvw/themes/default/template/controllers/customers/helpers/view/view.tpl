		...
		<div class="col-lg-6">
			<div class="panel clearfix">
				...
				<div class="form-horizontal">
					...
					<div class="row">
						<label class="control-label col-lg-3">{l s='Credit limit'}</label>
						<div class="col-lg-9">
							<p class="form-control-static">
            ...
            {if $products_groups AND count($products_groups)}
            <div class="panel">
              <div class="panel-heading">
                <i class="icon-archive"></i> {l s='Purchased products'}
              </div>
              <table class="table">
                <thead>
                  <tr>
                ...
                {foreach $products_groups as $product_group}
                  <tr >
                    <td><img src="{$product_group['image_url']}"></td>
                    <td>{$product_group['supplier_reference_base']}</td> 
                    <td>--</td>
                    <td>
                      --
                    </td>
                    <td>
                      {$product_group['quantity_total']}
                      <button class="btn btn-default" onclick="$(this).parents('table').find('.product-group-{$product_group['product_id']}').show();"
                       ><i class="icon-plus"></i></button>
                    </td>
                  </tr>

                  {foreach $product_group['products'] AS $key => $product}
                  <tr class="product-group-{$product_group['product_id']}" hidden onclick="document.location = '?tab=AdminOrders&amp;id_order={$product['id_order']|intval}&amp;vieworder&amp;token={getAdminToken tab='AdminOrders'}'">
