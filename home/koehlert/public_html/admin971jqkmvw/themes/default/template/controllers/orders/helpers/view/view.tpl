...
{block name="override_tpl"}
	...
	var txt_add_product_stock_clearance = "{l s='This is a clear stock dress. Are you sure it should be ordered?' js=1}";
	var txt_confirm = "{l s='Are you sure?' js=1}";
        var txt_ok = "{l s='Ok' js=1}";
        var txt_cancel = "{l s='Cancel' js=1}";
    ...
    var txt_order_state_need_due_date = "{l s='This status require due date' js=1}";
    var txt_add_invoice_and_print_now = "{l s='Do you want to print invoice?' js=1}";
    
    var txt_vat_confirmed = "{l s='Do you want to mark VAT confirmed?' js=1}";
    var txt_vat_unconfirmed = "{l s='Do you want to mark VAT unconfirmed?' js=1}";
    var txt_invoice_require_vat_customer_unconfirmed = "{l s='This invoice require customer to have confirmed VAT, but it is not' js=1}";
    var customer_vat_confirmed = {$customer->siret_confirmed|intval};
	</script>
	...
	<div class="row">
			...
			<div class="panel">
				...
				<div class="tab-content panel" id="order_view_content">
					...
					<div class="tab-pane active" id="status">
									...
									{foreach from=$history item=row key=key}
										{if ($key == 0)}
											<tr>
												...
												<td {if $row['need_date']|intval}id="orderCurrentStateDateDue"{/if} style="background-color:{$row['color']};color:{$row['text-color']}"
													>{if $row['need_date']|intval}{if $order->status_date_due!='0000-00-00'}{dateFormat date=$order->status_date_due full=false}{else}No due date{/if}{/if}</td>
                        ...
                        {if $can_edit}
						<!-- Change status form -->
						<form id="submitStateForm" action="{$currentIndex|escape:'html':'UTF-8'}&amp;vieworder&amp;token={$smarty.get.token}" method="post" class="form-horizontal well hidden-print">
							<div class="row">
								<div class="col-lg-6">
									<select id="id_order_state" class="chosen form-control" name="id_order_state">
									{foreach from=$states item=state}
										<option value="{$state['id_order_state']|intval}"{if isset($currentState) && $state['id_order_state'] == $currentState->id} selected="selected" disabled="disabled"{/if}
											data-need_date="{$state['need_date']|intval}"
								...
								<div class="col-lg-3"><!-- class=" fixed-width-xl" -->
									<div class="input-group">
										<input value="{if $order->status_date_due!='0000-00-00'}{$order->status_date_due}{/if}" autocomplete="off" id="order_state_due_date" type="text" name="order_state_due_date" class="datepicker" />
                        ...
                        <hr>
                        <div class="row">
                            <h4>{l s='Ignore this order in email "Attention: No invoices created longer than 5 days"'}</h4>
                            <div class="col-lg-3">
                                 <label><input type="checkbox" id="ignoreNoInvoiceEmail" {if $order->ignore_no_invoice_note}checked{/if}>&nbsp;{l s='Yes'}</label>
                            </div>
                            <div class="col-lg-9">
                                 <input value="{if $order->ignore_no_invoice_note}{$ignoreNoInvoiceComment.message|escape:'html':'UTF-8'}{/if}" 
                                    type="text" class="form-control" id="ignoreNoInvoiceEmailComment" placeholder="{l s='Comment'}"
                                    {if $order->ignore_no_invoice_note}readonly{/if} >
                                 <div class="text-muted" id="ignoreNoInvoiceEmailEmployee" {if $order->ignore_no_invoice_note == 0}hidden{/if}>
                                    by <em><span  class="text-italic"
                                        >{if $order->ignore_no_invoice_note}{$ignoreNoInvoiceComment.employee_name|escape:'html':'UTF-8'}{/if}</span></em>
                                 </div>
                            </div>
                        </div>
                        {/if}
		...
		<div class="col-lg-5">
					...
					<div class="row">
						<div class="col-xs-6" id="orderViewCustomerPanel">
							{if ($customer->isGuest())}
								...
								{/if}
							{else}
								{if $customer_has_inkasso_invoice}
									{assign var='customer_panel_style' value='inkassoCustomer'}
								{elseif $customer_overdue_invoices_unpaid_amount > 0}
									{assign var='customer_panel_style' value='notPaidInvoice'}
								{elseif $customer_invoices_unpaid_amount > $customer->credit_limit}
									{assign var='customer_panel_style' value='cstCredLmtOver'}
								{else}
									{assign var='customer_panel_style' value=''}
								{/if}
								<dl class="well list-detail {$customer_panel_style}">
									<dt>{l s='Email'}</dt>
									...
									{if Configuration::get('PS_B2B_ENABLE')}
										<dt class="order_view_customer_siret_label">
											...
											<dd class="order_view_customer_siret_text"
												><span class="order_view_customer_siret_value">{$customer->siret}</span> 
												<span data-active="{!$customer->siret_confirmed}" class="order_view_customer_siret_conf text-danger"><i class="icon-remove">&nbsp;</i></span>
												<span data-active="{$customer->siret_confirmed}" class="order_view_customer_siret_conf text-success"><i class="icon-check-circle">&nbsp;</i></span>
											</dd>
										<dt>{l s='APE'}</dt>
											<dd>{$customer->ape}</dd>
									{/if}
									<dt>{l s='Credit limit'}</dt>
									<dd>
									<div class="clearfix">
										<ul class="nav navbar-nav">
											<li>
											{displayPrice price=$customer_invoices_unpaid_amount}
											/
											<span id="customerCreditLimitValue" >{displayPrice price=$customer->credit_limit}</span>
											...
											<div class="dropdown">
												...
												<div>
													{if count($customer_credit_limit_history_list)}
													<table class="table">
													{foreach $customer_credit_limit_history_list as $customer_credit_limit_history_item}
														<tr>
															<td>{dateFormat date=$customer_credit_limit_history_item['date_add']}</td>
															<td>{$customer_credit_limit_history_item['employee_name']}</td>
															<td>{$customer_credit_limit_history_item['note']}</td>
														</tr>
													{/foreach}
													</table>
													{else}
									...
									{if isset($customer_exclusivity)}
									<dt>{l s='Exclusivity till'}</dt>
									<dd>{dateFormat date=$customer_exclusivity->date_end}</dd>
									{/if}
                                                                        {assign var="hookBurgelInfo" value={hook h="displayBurgelInfoOrder" order_id=$order->id}}
                                                                        {if ($hookBurgelInfo)}
                                                                            <dt>{l s='Burgel info'} <i class="icon-info-circle fa-2x" rel="#burgelHelp" id="burgelHelpIcon" ></i></dt>
                                                                            <dd>{hook h="displayBurgelInfoOrder" order_id=$order->id}}</dd>
                                                                        {/if}
						...
						<div class="col-xs-6" id="orderViewCustomerComments">
							...
							<div class="panel panel-sm">
                                ...
                                <table class="table table-condensed shop-comments-table" data-reference_id="{$customer->id}" data-reference_type="1">
                                  <tr>
                                    <th>{l s='Employee'}</th>
                                    <th>{l s='Date'}</th>
                                    <th>{l s='Comment'}</th>
        	...
        	<form action="index.php?controller=AdminShopCommentsComments&amp;token={getAdminToken tab='AdminShopCommentsComments'}&amp;action=save" method="post">
        		<div class="form-group">
        		...
        		<div class="form-group">
        			...
        			<button type="submit" class="btn btn-primary">{l s='Save note'}</button>
        		</div>
        	</form>
				...
				<div class="row">
					...
					<div class="tab-content panel">
								...
								<div class="well">
									<div class="row">
										<div class="col-sm-6">
                                            ...
                                            {if $can_edit}
											<hr>
											<button id="shippingLabelPrintAdd" class="btn btn-default">{l s='Print UPS label'}</button>
                                            {/if}
										</div>
                        ...
                        <form method="post" action="?tab=AdminAddresses&amp;deleteaddress&amp;id_order={$order->id}&amp;address_type=2&amp;back={$smarty.server.REQUEST_URI|urlencode}&amp;token={getAdminToken tab='AdminAddresses'}">
                          ...
                          {foreach from=$customer_addresses item=address}
                            <tr>
                              ...
                              <td>
                              {if ($address['id_address'] == $order->id_address_invoice) || ($address['id_address'] == $order->id_address_delivery)}
                                {l s='Used'}
                              {else}
                                <button value="{$address['id_address']}" name="id_address" class="btn btn-danger btn-xs customer-address-delete">{l s='Delete'}</button>
                              {/if}
                              </td>
                            </tr>
                          {/foreach}
						...
						<div id="message" class="form-horizontal">
									...
									<p class="help-block">
										<a href="{$link->getAdminLink('AdminOrderMessage')|escape:'html':'UTF-8'}">
											...
											<i class="icon-external-link"></i>
                                                        ...
                                                        <div class="form-group">
								<label class="control-label col-lg-3">{l s='Alternative E-mail addresses'}</label>
                                                                <div class="col-lg-9">
							...
							<div class="form-group">
								...
								<div class="col-lg-9">
                                    ...
                                    <div class="checkbox">
                                    <label>
                                    <input type="checkbox" value="{$document->id}" name="attach_invoice_id[]">
							...
							{/if} - {dateFormat date=$document->date_add}
                                    </label>
                                    </div>
                                    {/foreach}
			...
			<form class="container-command-top-spacing" action="{$current_index}&amp;vieworder&amp;token={$smarty.get.token|escape:'html':'UTF-8'}&amp;id_order={$order->id|intval}" method="post" onsubmit="return orderDeleteProduct('{l s='This product cannot be returned.'}', '{l s='Quantity to cancel is greater than quantity available.'}');">
				...
				<div class="panel">
						...
						<table class="table" id="orderProducts">
							<thead>
								<tr>
									...
									{/if}
									{if $stock_management}<th class="text-center"><span class="">{l s='Available quantity'}</span></th>{/if}
									{if $stock_management}<th class="text-center"><span class="" data-toggle="tooltip" title="{l s='Physical quantity is: Available quantity plus Not shipped products plus Deinballkleid pre orders. This formular is only applied if stock is less than 0'}">{l s='Physical quantity'}</span></th>{/if}
									<th>
										<span class="title_box ">{l s='Total'}</span>
										<small class="text-muted">{$smarty.capture.TaxMethod}</small>
									</th>
									<th><span class="title_box ">{l s='Note'}</span></th>
									<th><span class="title_box ">{l s='Note private'}</span></th>
					...
					{if Configuration::get('PS_ORDER_RETURN')}
						<a id="desc-order-standard_refund" class="btn btn-default" href="#refundForm">
							<i class="icon-exchange"></i>
							{* Mod. One name for similar functions *}
							{if $order->hasBeenShipped()}
								{l s='Return products'}
							{elseif $order->hasBeenPaid()}
								{l s='Return products'}{*l s='Standard refund'*}
							{else}
								{l s='Return products'}
							{/if}
						</a>
					...
					<div style="display: none;" class="standard_refund_fields form-horizontal panel">
						...
						<div class="row">
							<input type="submit" name="cancelProduct" {if count($customer_thread_message)}onClick="return confirm(readMessagesConfirmation)"{/if} value="{l s='Return products'}" class="btn btn-primary" />
						</div>
						{/if}
					</div>
	...
	<script type="text/javascript">
        ...
        var adminCanEdit = {$can_edit|intval};
		$(document).ready(function()
		{
            ...
            $.ajax({
                url: 'https://www.vipdress.de/admin123/index_service.php/dbk_ext_shop_delivery/get_dlvr_req_info/{php}echo Configuration::get('MSSS_CLIENT_SOURCE_ID'); {/php}/'+
                    '{$order->id}',
                dataType: 'html',
	...
	<div class="modal-dialog" role="document" id="orderDetailNoteModalDialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<button id="orderDetailNoteDialogSave" type="button" class="btn btn-primary">{l s='Save'}</button>
			</div>
		</div>
	</div>
</div>
                                ...
                                <div id="stockStatusHelp" class="hidden">
                                    <div class="bootstrap">
                                    {l s='Number of products still available for current order, after all orders with higher priority (more earlier orders) are shipped. Basically if you ship all orders with higher priority, then this number of products will remain for current order and for orders with less priority. Negative value means that x items should be shipped for orders with higher priority first and then current order many be shipped. This digit is unique for each order, it depends from order priority. It is not same as avail quantity in product details page. Quantity in product details page means quantity available for future orders or current stock state, that is same.'}
                                    <br>
                                    <span class="list-action-enable action-enabled"><i class="icon-check"></i></span> -- {l s='means product can be shipped right now'}<br />
                                    <span class="list-action-enable action-enabled colorOrange"><i class="icon-check"></i></span> -- {l s='means product available, but it is in one of dbk shops now'}<br />
                                    <span class="list-action-enable action-disabled stockStatusExpDelivery"><i class="icon-remove"></i></span> -- {l s='means product not available now, you can see expected delivery if you move mouse on icon'}
                                    </div>
                                </div>
{/block}
