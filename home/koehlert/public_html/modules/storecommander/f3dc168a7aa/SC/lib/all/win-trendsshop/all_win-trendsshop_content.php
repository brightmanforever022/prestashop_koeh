        ...
        foreach($conditions as $field => $condition) {
            ...
            switch($field) {
                ...
                case'date_end':
                    ...
                    break;
                case'payment_method':
                    $modules_names = Db::getInstance()->executeS('SELECT name FROM `'._DB_PREFIX_.'module` WHERE `id_module` IN ('.pSQL($ids).')');
                    $ids=array();
                    foreach($modules_names as $mod) {
                        $ids[] = $mod['name'];
                    }
                    $ids = implode('","',$ids);
                    $return .= ' AND o.module IN ("'.$ids.'")';
                    break;
                case'carrier':
                    $sql = 'SELECT id_carrier 
                            FROM `'._DB_PREFIX_.'carrier` 
                            WHERE `id_reference` IN ('.pSQL($ids).')';
                    $return .= ' AND o.id_carrier IN ('.$sql.')';
                    break;
                case'country':
                    $return .= ' AND addr.id_country IN ('.pSQL($ids).')';
                    break;
                case'customer_group':
                    $return .= ' AND c.id_default_group IN ('.pSQL($ids).')';
                    break;
                default:
                    $return .= ' AND o.id_'.pSQL($field).' IN ('.pSQL($ids).')';
                    break;
            }
...
<script type="text/javascript">
    ...
    dhx.i18n.setLocale("pivot", {
        ...
        ok: '<?php echo _l("Ok") ?>',
        date: {
            month_full: month_full_arr,
            month_short: month_short_arr,
            day_full: day_full_arr,
            day_short: day_short_arr,
            quarter: quarter_arr,
            week: '<?php echo _l("Week") ?>'
        }
    });
