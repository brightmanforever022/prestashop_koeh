				...
				if($segment->type=="manual")
				{
					...
					FROM "._DB_PREFIX_."orders o
							INNER JOIN "._DB_PREFIX_."order_detail od ON (o.id_order = od.id_order)
								INNER JOIN "._DB_PREFIX_."sc_segment_element se ON (se.id_element = od.product_id AND se.type_element='product' AND se.id_segment='".intval($auto_params["id_segment"])."')";
				}
				elseif($segment->type=="auto")
				{
					...
					if(is_array($res_segment) && !empty($res_segment))
					{
						...
						$sql = "SELECT DISTINCT(o.id_order)
							FROM "._DB_PREFIX_."orders o
									INNER JOIN "._DB_PREFIX_."order_detail od ON (o.id_order = od.id_order)
							WHERE od.product_id IN (".pSQL(implode(",", $ids)).")";
					}
				}
				if(!empty($sql))
				{
