...
if(!empty($post_action) && $post_action=="do_check")
{
	...
	if(!empty($res) && count($res)>0)
	{
		...
		<script type="text/javascript">
			...
			dhxlSCExtCheck.tabbar.cells("table_TRP_CAR_GHOST_CARRIER").attachStatusBar({text: '<?php echo _l('Count rows:')." ".count($res) ; ?>', height: 25 });
...
elseif(!empty($post_action) && $post_action=="delete_carriers")
{
	...
	if(!empty($post_ids))
	{
		$sql = "DELETE FROM "._DB_PREFIX_."carrier_lang WHERE id_carrier IN (".psql($post_ids).")";
