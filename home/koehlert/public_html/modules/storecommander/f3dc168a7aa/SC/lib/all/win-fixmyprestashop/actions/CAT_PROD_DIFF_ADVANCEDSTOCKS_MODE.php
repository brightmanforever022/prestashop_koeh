...
if(!empty($post_action) && $post_action=="do_check")
{
	...
	if(!empty($res) && count($res)>0)
	{
		$results = "KO";
		ob_start();
		$id_lang=intval(Tools::getValue('id_lang', SCI::getConfigurationValue("PS_LANG_DEFAULT")));
		?>
		<script type="text/javascript">
	
			var tbDiffAdvancedstocksMode = dhxlSCExtCheck.tabbar.cells("table_CAT_PROD_DIFF_ADVANCEDSTOCKS_MODE").attachToolbar();
			tbDiffAdvancedstocksMode.addButton("goto", 0, "", "lib/img/zoom.png", "lib/img/zoom.png");
			...
			dhxlSCExtCheck.tabbar.cells("CAT_PROD_DIFF_ADVANCEDSTOCKS_MODE").attachStatusBar({text: '<?php echo _l('Count rows:')." ".count($res) ; ?>', height: 25 });
