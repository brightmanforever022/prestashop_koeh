...
if(!empty($post_action) && $post_action=="do_check")
{
	...
	if(!empty($res) && count($res)>0)
	{
		...
		<script type="text/javascript">
			...
			tbGhostStockAvailable.addButton("selectall", 0, "", "lib/img/application_lightning.png", "lib/img/application_lightning.png");
			tbGhostStockAvailable.setItemToolTip('selectall','<?php echo _l('Select all')?>');
			tbGhostStockAvailable.addButton("delete", 0, "", "lib/img/delete.gif", "lib/img/delete.gif");
			...
			dhxlSCExtCheck.tabbar.cells("table_CAT_STK_GHOST_STOCK_AVAILABLE").attachStatusBar({text: '<?php echo _l('Count rows:')." ".count($res) ; ?>', height: 25 });
...
elseif(!empty($post_action) && $post_action=="delete_stockavailable")
{
	...
	if(!empty($post_ids))
	{
		...
		foreach($ids as $id)
		{
			...
			list($id_product,$id_product_attribute)=Db::getInstance()->getRow($sql);
