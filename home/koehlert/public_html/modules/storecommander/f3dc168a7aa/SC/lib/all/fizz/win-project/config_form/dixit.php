...
$ret = sc_file_post_contents('http://api.storecommander.com/Dixit/getLangs/', $posts, $headers,true);
if(!empty($ret))
{
...
foreach ($langs as $lang)
{
    ...
    <?php if(in_array($project["status"],array("2","3","4","7","101","103","107","113"))) {
        ...
        <?php } ?>
        <br/>
    <?php } elseif(in_array($project["status"],array("1"))) { ?>
    ...
    <div class="div_form">
        ...
        <select id="level">
            ...
            <option value="normal" <?php if(!empty($params["level"]) && $params["level"]=="normal") echo "selected"; ?>><?php echo _l('Pro'); ?></option>
            <option value="advanced" <?php if(!empty($params["level"]) && $params["level"]=="advanced") echo "selected"; ?>><?php echo _l('Expert (with proofreading)'); ?></option>
        </select>
    </div>
    <div class="div_form">
        <div class="form_label">
        ...
        <select id="lang_source">
            ...
            foreach($valid_langs as $lang) { ?>
                <option value="<?php echo $lang["language_id"]."_".$lang["iso_code_2"]; ?>" <?php if(!empty($params["lang_source"]) && $lang["language_id"]."_".$lang["iso_code_2"]==$params["lang_source"]) echo "selected"; ?>><?php echo $lang["content"]; ?></option>
            <?php } ?>
        </select>
    </div>
    <div class="div_form">
        ...
        <select id="lang_translation">
            ...
            <?php
            foreach($valid_langs as $lang) { ?>
                <option value="<?php echo $lang["language_id"]."_".$lang["iso_code_2"]; ?>" <?php if(!empty($params["lang_translation"]) && $lang["language_id"]."_".$lang["iso_code_2"]==$params["lang_translation"]) echo "selected"; ?>><?php echo $lang["content"]; ?></option>
            <?php } ?>
        </select>
    </div>
    <div class="div_form">
        ...
        foreach($enabled_sources as $id=>$name) { ?>
            <input type="checkbox" class="chk colspan fields_source" value="<?php echo $id; ?>" <?php if(in_array($id,$sources)) echo "checked"; ?> style="" /> <?php echo $name; ?><br/>
        <?php } ?>
    </div>
    <br/>
