...
$id_segment=intval(Tools::getValue('id_segment'),0);
...
<body>
<div class="form">
	<div id="div_config" style="width: <?php if(!empty($html_segment)) echo "48%;  padding-right: 2%; border-right: 1px solid #A4BED4"; else echo "99%"; ?>; float:left;">
		<div style="width: 38%; float:left;">
			<strong><?php echo _l("Interface:") ?></strong><br/>
			<select id="form_segment_access" multiple="multiple" style="width: 100%; height: 6em;">
				<option value="catalog" <?php if(in_array("catalog", $in_access)) echo "selected"; ?>><?php echo _l("Catalog") ?></option>
				<option value="orders" <?php if(in_array("orders", $in_access)) echo "selected"; ?>><?php echo _l("Orders") ?></option>
				<option value="customers" <?php if(in_array("customers", $in_access)) echo "selected"; ?>><?php echo _l("Customers") ?></option>
				<?php if(version_compare(_PS_VERSION_, '1.4.0.0', '>=')) { ?>
				<option value="customer_service" <?php if(in_array("customer_service", $in_access)) echo "selected"; ?>><?php echo _l("Customer service") ?></option>
				<?php } ?>
			</select>
		...
		<select id="form_segment_auto_file" style="width: 100%;">
			<?php /*?><option value="catalog" <?php if(in_array("catalog", $in_access)) echo "selected"; ?>><?php echo _l("Catalog") ?></option>*/ ?>
			<?php 
			$type_segments = array();
			foreach(SegmentHook::$listFiles as $file)
			{
				if(file_exists(SC_SEGMENTS_DIR.$file))
				{
					...
					if(
						version_compare(_PS_VERSION_, '1.4.0.0', '>=')
						||
						(version_compare(_PS_VERSION_, '1.4.0.0', '<') && strpos(strtolower($class_name), "ticket")===false)
					)
					{
			...
			foreach($type_segments as $type_segment)
			{
				...
				?>
				<option value="<?php echo $class_name; ?>" <?php if($segment->auto_file==$class_name) echo "selected"; ?>><?php echo _l($instance->name); ?></option>
				<?php
			}
			?>
		</select>
...
function propertiesSave(){
    ...
    $.each(auto_params_temp, function (id, value){
        auto_params[auto_params.length] = {"name":id , "value":value};
    });
...
$(document).ready(function(){
	/*$("#form_segment_auto_file option").click(function(){
		var value = $(this).val();
		$("#form_segment_auto_file").val("");
		$("#form_segment_auto_file").val(value);
	});*/
