...
if(!empty($post_action) && $post_action=="do_check")
{
    ...
    if(!empty($ref_found)) {
        ...
        $sql = 'SELECT pa.id_product_attribute,pa.id_product,p.id_category_default,pa.reference,pl.name, p.active '.(version_compare(_PS_VERSION_, '1.5.0.0', '>=') ? ' , ps.active ': '').'
    ...
    if(!empty($res) && count($res)>0)
    {
        ...
        <script type="text/javascript">
            ...
            tbCombinationSameRef.addButton("gotocatalog", 0, "", "lib/img/combinations.gif", "lib/img/combinations.gif");
            tbCombinationSameRef.setItemToolTip('gotocatalog','<?php echo _l('Go to the combination in catalog.')?>');
            tbCombinationSameRef.addButton("exportcsv", 0, "", "lib/img/page_excel.png", "lib/img/page_excel.png");
