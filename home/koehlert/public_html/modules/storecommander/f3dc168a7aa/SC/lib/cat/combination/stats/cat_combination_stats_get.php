...
<?php
	...
	$list_id_product_attribute = Tools::getValue('id_product_attribute',0);
	...
	for ($i = 30; $i >=0 ; $i--) {
	    ...
	    if(!empty($view_total_price)) {
	 	...
	 	$sql ="SELECT ".pSQL($sum_sql).", DAY(o.date_add) as day FROM "._DB_PREFIX_."orders o
	 				LEFT JOIN "._DB_PREFIX_."order_detail od ON (o.id_order = od.id_order)
	 				WHERE o.valid = 1
	 				AND od.product_attribute_id IN (".pSQL($list_id_product_attribute).")
...
?>
		...
		myBarChart3 =  new dhtmlXChart({
			...
			yAxis:{
				title:"<?php echo _l('Sales').(!empty($view_total_price) ? ' ('._l('Total combinations excl. tax').')': ''); ?>"
			}
		});
...
<?php
	...
	for ($i = $currentMonth; $i < 13; $i++) {
	 	$sql ="SELECT ".pSQL($sum_sql)." FROM "._DB_PREFIX_."orders o
	 				LEFT JOIN "._DB_PREFIX_."order_detail od ON (o.id_order = od.id_order)
	 				WHERE o.valid = 1
	 				AND od.product_attribute_id IN (".pSQL($list_id_product_attribute).")
