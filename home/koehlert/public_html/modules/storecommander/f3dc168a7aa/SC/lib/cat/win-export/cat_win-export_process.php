						...
						foreach($features AS $feature)
						{
							...
							if($multipleFeatureEnabled && !empty($featuresListByLang[$lang['id_lang']][$featuresListNameDefault[$feature['id_feature']]])) {
							    if(!empty($featuresListByLang[$lang['id_lang']][$featuresListNameDefault[$feature['id_feature']]]['value'])) {
                                    $featuresListByLang[$lang['id_lang']][$featuresListNameDefault[$feature['id_feature']]]['value'] .= $separator;
                                }
									...
									if(SCAS)
									{
										...
										switch($f['name'])
										{
                                            ...
                                            case 'additional_delivery_times':
                                                $field=intval($p->additional_delivery_times);
                                                break;
                                            ...
                                            case 'low_stock_alert':
                                                $field=intval($p->low_stock_alert);
                                                break;
                                            case 'low_stock_threshold':
                                                $field=intval($p->low_stock_threshold);
                                                break;
                                            ...
                                            case 'stock_value':
                                                if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
                                                {
                                                    ...
                                                    if($no_scas)
                                                    {
                                                        ...
                                                        {
                                                            $attrs = $p->getAttributeCombinations($id_lang);
                                                            foreach ($attrs as $attr)
                                                            {
                                                                $id_pa = $attr["id_product_attribute"];
                                                                $qty=StockAvailable::getQuantityAvailableByProduct($p->id, $id_pa, (SCMS && $selected_shops_id>0?(int)$selected_shops_id:null));
                                                                $field+=$qty*$p->getPrice(false,$id_pa, 6, null, false, false);
                                                            }
                                                        }
                                                     }
