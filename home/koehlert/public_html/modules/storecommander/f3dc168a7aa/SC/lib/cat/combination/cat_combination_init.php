...
?>
	...
	prop_tb.setItemToolTip('combi_sortByPosition','<?php echo _l('Sort by position',1)?>');
     var options_for_references = [
        ...
        ['or_prodref_auto', 'obj', '<?php echo _l('product reference - unique id')?>', ''],
        ['or_prodref_attrid', 'obj', '<?php echo _l('product reference - id_product_attribute')?>', ''],
        ['or_prodref_auto_name', 'obj', '<?php echo _l('product reference - unique id - name')?>', '']
    ]
    prop_tb.addButtonSelect('reference_generator',100,'<?php echo _l('Combination references')?>',options_for_references,'lib/img/key_add.png','lib/img/key_add.png',false,true);
		...
		if (needInitCombinations)
		{
            ...
            switch("<?php echo _s("CAT_PROD_COMBI_DEFAULT_SUBCOMBI"); ?>"){
                case "image":
                    ...
                    default_sub_combi = ["<?php echo _l('Images')?>","picture.png"];
                    break;
                case "shopshare":
                    actual_subproperties = "combi_shop";
                    default_sub_combi = ["<?php echo _l('Multistore sharing manager')?>","sitemap_color.png"];
                    break;
                case "specificprice":
                    actual_subproperties = "combi_specificprices";
                    default_sub_combi = ["<?php echo _l('Specific prices')?>","text_list_numbers.png"];
                    break;
                case "stats":
                    actual_subproperties = "combination_stats";
                    default_sub_combi = ["<?php echo _l('Stats')?>","chart_curve.png"];
                    break;
                case "supplier":
                    actual_subproperties = "combi_suppliers";
                    default_sub_combi = ["<?php echo _l('Suppliers')?>","package.png"];
                    break;
                case "warehouseshare":
                    actual_subproperties = "combi_warehouses";
                    default_sub_combi = ["<?php echo _l('Warehouses')?>","picture.png"];
                    break;
                default:
                    actual_subproperties = "combi_images";
                    default_sub_combi = ["<?php echo _l('Images')?>","building.png"];
            }
            prop_tb.combi_subproperties_tb.addButtonSelect("combiSubProperties", 0, default_sub_combi[0], opts, "lib/img/"+default_sub_combi[1], "lib/img/"+default_sub_combi[1],false,true);
...
?>
        ...
        */
        if (id=='or_delete'
            || id=='or_prodref_auto'
            || id=='or_prodref_attrid'
            || id=='or_prodref_auto_name')
        {
    ...
    if (confirm('<?php echo _l('Are you sure you want to do this action ?',1)?>'))
    {
        ...
        prop_tb._combinationsGrid.forEachRow(function(rId)
        {
            ...
            if(id=='or_prodref_auto_name') {
                ...
                var new_ref = String(product_ref+'-'+id_auto+'-'+name_row.join('-'));
                checkDuplicateRefBeforeSave(product_id,rId,idxProductAttributeRef,new_ref);
            }
            id_auto++;
        });
...
function displayCombinations(callback,forceGroups)
{
	if (typeof prop_tb._combinationsGrid != 'undefined')
	{
		...
		prop_tb._combinationsGrid.load("index.php?ajax=1&act=cat_combination_get&id_product="+lastProductSelID+"&forceGroups="+forceGroups+"&sortByPosition="+$.cookie('combi_sort_by_position')+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function()
				{
