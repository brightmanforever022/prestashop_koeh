	...
	if(!empty($res) && count($res)>0)
	{
		...
		foreach ($res as $category) {
			...
			if(!$is_root && !$is_recycle_bin)
			{
				...
				xml = xml+'<userdata name=\"not_deletable\">".intval($not_deletable)."</userdata>';
				xml = xml+'<userdata name=\"is_home\">".intval($is_home)."</userdata>';
				xml = xml+'<userdata name=\"is_root\">".intval($is_root)."</userdata>';
		...
		<script type="text/javascript">
			...
			dhxlSCExtCheck.tabbar.cells("table_CAT_CAT_GHOST_CAT_MS").attachStatusBar({text: '<?php echo _l('Count rows:')." ".count($res) ; ?>', height: 25 });
