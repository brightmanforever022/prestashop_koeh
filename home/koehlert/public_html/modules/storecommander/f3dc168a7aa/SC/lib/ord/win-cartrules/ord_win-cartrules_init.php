...
// Properties
    ...
    var opts = new Array();
    ord_wincartrules_prop_tb.addButtonSelect("ord_prop_subproperties", 100, "<?php echo _l('Edit the discount voucher')?>", opts, "lib/img/tag_blue.png", "lib/img/application_form_magnify.png",false,true);
    ...
    foreach ($sub_files as $sub_item) {
        if ($sub_item != '.' && $sub_item != '..') {
            if (is_dir(SC_DIR.$parent_folder.'/'.$sub_item) && file_exists(SC_DIR.$parent_folder.'/'.$sub_item.'/'.$prefix_file.$sub_item.'_init.php'))
            {
