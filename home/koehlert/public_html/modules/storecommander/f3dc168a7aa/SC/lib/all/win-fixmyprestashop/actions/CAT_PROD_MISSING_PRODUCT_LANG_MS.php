...
if(!empty($post_action) && $post_action=="do_check")
{
	/*$sql = "SELECT p.*, s.id_shop, s.name AS shop_name
			...
			WHERE 
				p.id_product NOT IN (SELECT pl.id_product FROM "._DB_PREFIX_."product_lang pl WHERE pl.id_shop = ps.id_shop)";
	...
	if(!empty($res) && count($res)>0)
	{
		...
		<script type="text/javascript">
			...
			tbMissingProductLang.addButton("selectall", 0, "", "lib/img/application_lightning.png", "lib/img/application_lightning.png");
			tbMissingProductLang.setItemToolTip('selectall','<?php echo _l('Select all')?>');
			tbMissingProductLang.addButton("delete", 0, "", "lib/img/delete.gif", "lib/img/delete.gif");
			tbMissingProductLang.setItemToolTip('delete','<?php echo _l('Remove products from shops')?>');
			tbMissingProductLang.addButton("add", 0, "", "lib/img/add.png", "lib/img/add.png");
			...
			dhxlSCExtCheck.tabbar.cells("table_CAT_PROD_MISSING_PRODUCT_LANG_MS").attachStatusBar({text: '<?php echo _l('Count rows:')." ".count($res) ; ?>', height: 25 });
