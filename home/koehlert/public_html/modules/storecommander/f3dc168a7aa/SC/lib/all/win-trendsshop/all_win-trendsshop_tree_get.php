                        ...
                        if(!empty($found)) {
                            foreach($found as $res) {
                                ...
                                if(version_compare(_PS_VERSION_, '1.5.0.0', '>=')) {
                                    ...
                                    $id_module = (int)Db::getInstance()->getValue('SELECT `id_module` FROM `'._DB_PREFIX_.'module` WHERE `name` = "'.pSQL($name).'"');
                                }
                                if(version_compare(_PS_VERSION_, '1.5.0.0', '>=')) {
                                ...
                                $return .= createRow((int)$id_module,$module_name,$filter_key);
                            }
                        }
                        break;
                    case'carrier':
                        ...
                        break;
                    case'manufacturer':
                        ## à transformer en requete sql quand on utilisera ce champ
                        $manufacturers = Manufacturer::getManufacturers(false, $id_lang, true);
                        $return .= returnData($manufacturers, $filter_key);
                        break;
                    case'supplier':
                        ## à transformer en requete sql quand on utilisera ce champ
                        $suppliers = Supplier::getSuppliers(false, $id_lang, true);
                        $return .= returnData($suppliers, $filter_key);
                        break;
                    case'customer_group':
