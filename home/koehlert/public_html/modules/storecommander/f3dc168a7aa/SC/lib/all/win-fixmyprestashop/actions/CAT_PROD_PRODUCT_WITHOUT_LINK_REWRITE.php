...
if(!empty($post_action) && $post_action=="do_check")
{
	...
	if(!empty($res) && count($res)>0)
	{
		...
		<script type="text/javascript">
			...
			tbProductWithoutLinkRewrite.addButton("selectall", 0, "", "lib/img/application_lightning.png", "lib/img/application_lightning.png");
			tbProductWithoutLinkRewrite.setItemToolTip('selectall','<?php echo _l('Select all')?>');
			tbProductWithoutLinkRewrite.addButton("generate", 0, "", "lib/img/cog_go.png", "lib/img/cog_go.png");
			...
			dhxlSCExtCheck.tabbar.cells("table_CAT_PROD_PRODUCT_WITHOUT_LINK_REWRITE").attachStatusBar({text: '<?php echo _l('Count rows:')." ".count($res) ; ?>', height: 25 });
