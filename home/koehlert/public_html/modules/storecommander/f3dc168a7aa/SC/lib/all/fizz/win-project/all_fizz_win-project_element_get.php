    ...
    if(!empty($project["list_items"]) && $project["list_items"]!="-")
    {
        ...
        foreach($res AS $id)
        {
            ...
            if($project["type"]=="dixit")
            {
                ...
                if(SCMS)
                {
                    $element = new Product($id, false);
                    $element = new Product($id, false, null, $element->id_shop_default);
                }
                else
            ...
            elseif($project["type"]=="cutout")
            {
                ...
                if (file_exists(SC_PS_PATH_REL."img/p/".getImgPath(intval($element->id_product),intval($element->id),_s('CAT_PROD_GRID_IMAGE_SIZE')))) {
                    $element_info = "<img src='".SC_PS_PATH_REL."img/p/".getImgPath(intval($element->id_product),intval($element->id),_s('CAT_PROD_GRID_IMAGE_SIZE'))."'/>";
                } else {
