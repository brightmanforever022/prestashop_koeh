...
function removeInShop($product_id, $shop_id)
{
	if(!empty($product_id) && !empty($shop_id))
	{
		...
		$sql = "DELETE FROM `"._DB_PREFIX_."product_shop` WHERE `id_product` = '".intval($product_id)."' AND id_shop = '".intval($shop_id)."'";
		Db::getInstance()->Execute($sql);

		// stock_available
		// delete row pour ce product et ce shop
		$sql = "DELETE FROM `"._DB_PREFIX_."stock_available` WHERE `id_product` = '".intval($product_id)."' AND id_shop = '".intval($shop_id)."'";
		Db::getInstance()->Execute($sql);

		// product_lang
		// delete row pour ce product et ce shop
		$sql = "DELETE FROM `"._DB_PREFIX_."product_lang` WHERE `id_product` = '".intval($product_id)."' AND id_shop = '".intval($shop_id)."'";
		Db::getInstance()->Execute($sql);
		...
		foreach($attributes as $attribute)
		{
			...
			{
				$sql = "DELETE FROM `"._DB_PREFIX_."product_attribute_shop` WHERE `id_product_attribute` = '".intval($attribute["id_product_attribute"])."' AND id_shop = '".intval($shop_id)."'";
				Db::getInstance()->Execute($sql);
			}
		}
		...
		foreach($images as $image)
		{
			...
			{
				$sql = "DELETE FROM `"._DB_PREFIX_."image_shop` WHERE `id_image` = '".intval($image["id_image"])."' AND id_shop = '".intval($shop_id)."'";
				Db::getInstance()->Execute($sql);
			}
		}
		...
		foreach($features as $feature)
		{
			if(!empty($feature["id_feature"]))
			{
				...
				{
					$sql = "SELECT id_feature_value FROM `"._DB_PREFIX_."feature_product` WHERE `id_feature` = '".intval($feature->id)."' AND id_product = '".intval($product_id)."'";
					$values = Db::getInstance()->ExecuteS($sql);
					foreach($values as $value)
					{
                        // si feature_value n'existe pas pour un autre produit
                        $sql = "SELECT id_feature_value FROM `"._DB_PREFIX_."feature_product` WHERE `id_feature_value` = '".intval($value["id_feature_value"])."' AND id_product != '".intval($product_id)."'";
					...
					$sql = "DELETE FROM `"._DB_PREFIX_."feature_product` WHERE `id_feature` = '".intval($feature->id)."' AND id_product = '".intval($product_id)."'";
					Db::getInstance()->Execute($sql);
				}
			}
		}
		...
		$sql = "SELECT id_supplier FROM `"._DB_PREFIX_."product_supplier` WHERE id_product = '".intval($product_id)."' GROUP BY id_supplier";
		...
		foreach($suppliers as $supplier)
		{
			if(!empty($supplier) && !empty($supplier["id_supplier"]))
			{
				...
				if(count($associated_shops)==1 && $associated_shops[0]==$shop_id)
				{
					$sql = "DELETE FROM `"._DB_PREFIX_."supplier_product` WHERE `id_supplier` = '".intval($supplier->id)."' AND id_product = '".intval($product_id)."'";
					Db::getInstance()->Execute($sql);
				}
			}
		}
		...
		$sql = "SELECT id_warehouse FROM `"._DB_PREFIX_."warehouse_product_location` WHERE id_product = '".intval($product_id)."' GROUP BY id_warehouse";
		...
		foreach($warehouses as $warehouse)
		{
			if(!empty($warehouse["id_warehouse"]))
			{
				...
				if(count($associated_shops)==1 && $associated_shops[0]==$shop_id)
				{
					$sql = "DELETE FROM `"._DB_PREFIX_."warehouse_product_location` WHERE `id_warehouse` = '".intval($warehouse->id)."' AND id_product = '".intval($product_id)."'";
					Db::getInstance()->Execute($sql);
				}
			}
		}
						...
						switch($action_upd)
						{
							...
							case 'active':
								foreach($ids as $id)
								{
									...
									{
										$sql = "UPDATE `"._DB_PREFIX_."product_shop` SET active='".(int)$value."' WHERE `id_product` = '".intval($id)."' AND id_shop = '".$id_shop."'";
										Db::getInstance()->Execute($sql);
									}
								}
								break;
							...
							case 'mass_active':
								...
								foreach($shops as $shop)
								{
									foreach($ids as $id)
									{
										...
										{
											$sql = "UPDATE `"._DB_PREFIX_."product_shop` SET active='".(int)$value."' WHERE `id_product` = '".intval($id)."' AND id_shop = '".$shop."'";
											Db::getInstance()->Execute($sql);
										}
									}
								}
