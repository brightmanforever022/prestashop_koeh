<script type="text/javascript">
    ...
    eSP_projects_grid_sb.setText('<div id="eSP_projects_started_statusqueue" style="color: #ff0000; font-weight: bold;display:none; margin-right: 20px;"><img src="lib/img/ajax-loader16.gif" style="height: 10px;" /> <?php echo _l("We are creating the elements at SC service's for the project",1); ?> <span></span></div><div id="eSP_projects_checkstatus" style="color: #ff0000; font-weight: bold;display:none;"><img src="lib/img/ajax-loader16.gif" style="height: 10px;" /> <?php echo _l("We are checking the status of the project and importing data.",1); ?></div>');
    ...
    eSP_projects_tb.addButton("refresh", 100, "", "lib/img/arrow_refresh.png", "lib/img/arrow_refresh.png");
    eSP_projects_tb.setItemToolTip('refresh','<?php echo _l('Refresh grid')?>');
    eSP_projects_tb.addButtonTwoState("display_archived", 100, "", "lib/img/folder_grey.png", "lib/img/folder_grey.png");
    eSP_projects_tb.setItemToolTip('display_archived','<?php echo _l('Show archived projects')?>');
    eSP_projects_tb.addButtonTwoState("group_by_type", 100, "", "lib/img/arrow_in.png", "lib/img/arrow_in.png");
    eSP_projects_tb.setItemToolTip('group_by_type','<?php echo _l('Group projects by type')?>');
    eSP_projects_tb.addSeparator('sep1', 100);
    <?php if(KAI9DF4!=1) { ?>
    eSP_projects_tb.addButton("add", 100, "", "lib/img/add.png", "lib/img/add.png");
    eSP_projects_tb.setItemToolTip('add','<?php echo _l('Create project')?>');
    eSP_projects_tb.addButton("delete", 100, "", "lib/img/delete.gif", "lib/img/delete.gif");
    eSP_projects_tb.setItemToolTip('delete','<?php echo _l('Delete project')?>');
    eSP_projects_tb.addSeparator('sep2', 100);
    eSP_projects_tb.addButton("checkStatus", 100, "", "lib/img/tick.png", "lib/img/tick.png");
    ...
    eSP_projects_tb.addButton("archive", 100, "", "lib/img/folder_grey_go.png", "lib/img/folder_grey_go.png");
    ...
    eSP_projects_tb.attachEvent("onClick",function(id){
        ...
        if(id=="add")
        {
            if (!dhxWins.isWindow('weServicesProjectCreate'))
            {
                weServicesProjectCreate = dhxWins.createWindow('weServicesProjectCreate', (($(window).width()-300)/2), (($(window).height()-240)/2), 300, 240);
                weServicesProjectCreate.setIcon('lib/img/add.png','../../../lib/img/add.png');
                weServicesProjectCreate.setText('<?php echo _l('e-Services', 1)." - "._l('Create a project', 1); ?>');
                eSP_Create = weServicesProjectCreate.attachForm([
                    {type:"fieldset", name:"data", label:"<?php echo _l("My new project"); ?>", inputWidth:"270",
                        list:[
                            {type:"input",    name: "nameProject", label:"<?php echo _l("Project name"); ?>"},
                            {type: "combo",   name:"typeProject", label: "<?php echo _l("Project type"); ?>", inputWidth:200, options:[
                                {text: "-- <?php echo _l("Choose a type"); ?> --",      value: ""},
                                {text: "<?php echo _l("Product translation Pro"); ?>", value: "dixit"},
                            ]},
                            {type:"button",   name:"createProject", value:"<?php echo _l("Create"); ?>"}]
                    }
                ]);
                eSP_Create.attachEvent("onButtonClick", function(id){
                    if(id=="createProject")
                    {
                        var value = eSP_Create.getItemValue("typeProject");
                        var name = eSP_Create.getItemValue("nameProject");
                        if(name!=undefined && name!=null && name!="" && name!=0)
                        {
                            if(value!=undefined && value!=null && value!="" && value!=0)
                            {
                                $.post("index.php?ajax=1&act=all_fizz_win-project_update&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{'action': 'add','type': value, 'name': name},function(data)
                                {
                                    weServicesProjectCreate.close();
                                    displayProjects();
                                });
                            }
                        }
    ...
    var opts = [
        ['eservices_cutout', 'obj', '<?php echo _l('Cut the selected image out',1)?>', 'lib/img/eservices_cp.png'],
    ];
    eSP_itemslist_tb.addButtonSelect("eservices_list", 0, "",opts, "lib/img/ruby.png", "lib/img/ruby.png",false,true);
    eSP_itemslist_tb.setItemToolTip("eservices_list","<?php echo _l('e-Services',1)?>");
    eSP_itemslist_tb.addSeparator('sepExt', 0);
    eSP_itemslist_tb.addButton("exportcsv", 0, "", "lib/img/page_excel.png", "lib/img/page_excel.png");
    ...
    eSP_itemslist_tb.addButton("delete", 0, "", "lib/img/delete.gif", "lib/img/delete.gif");
    eSP_itemslist_tb.setItemToolTip('delete','<?php echo _l('Remove selected items')?>');
    eSP_itemslist_tb.addButton("selectall", 0, "", "lib/img/application_lightning.png", "lib/img/application_lightning.png");
    eSP_itemslist_tb.setItemToolTip('selectall','<?php echo _l('Select all')?>');
    eSP_itemslist_tb.addButton("refresh", 0, "", "lib/img/arrow_refresh.png", "lib/img/arrow_refresh.png");
    ...
    function displayESPElements()
    {
        if(selected_project!=undefined && selected_project!=null && selected_project!="")
        {
            ...
            eSP_itemslist_grid.loadXML("index.php?ajax=1&act=all_fizz_win-project_element_get&id_project="+selected_project+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function()
            {
                col_eSP_itemslist.progressOff();

                eSP_itemslist_grid_sb.setText(eSP_itemslist_grid.getRowsNum()+' '+(eSP_itemslist_grid.getRowsNum()>1?'<?php echo _l('elements')?>':'<?php echo _l('element')?>'));
