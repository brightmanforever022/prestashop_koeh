...
if(!empty($post_action) && $post_action=="do_check")
{
	...
	if(!empty($res) && count($res)>0)
	{
		...
		<script type="text/javascript">
			...
			tbGhostAttributeGroup.addButton("selectall", 0, "", "lib/img/application_lightning.png", "lib/img/application_lightning.png");
			tbGhostAttributeGroup.setItemToolTip('selectall','<?php echo _l('Select all')?>');
			tbGhostAttributeGroup.addButton("delete", 0, "", "lib/img/delete.gif", "lib/img/delete.gif");
			tbGhostAttributeGroup.setItemToolTip('delete','<?php echo _l('Delete incomplete attribute groups')?>');
			tbGhostAttributeGroup.addButton("add", 0, "", "lib/img/add.png", "lib/img/add.png");
			...
			xml = xml+'  	<cell><![CDATA[<?php if(!empty($attributs) && count($attributs)>0) echo _l('Yes'); else echo _l('No'); ?>]]></cell>';
			...
			dhxlSCExtCheck.tabbar.cells("table_CAT_ATTR_GHOST_GROUP_ATTRIBUTE").attachStatusBar({text: '<?php echo _l('Count rows:')." ".count($res) ; ?>', height: 25 });
...
elseif(!empty($post_action) && $post_action=="delete_attribute_groups")
{
	...
	if(!empty($post_ids))
	{
		$sql = "DELETE FROM "._DB_PREFIX_."attribute_group_lang WHERE id_attribute_group IN (".psql($post_ids).")";
