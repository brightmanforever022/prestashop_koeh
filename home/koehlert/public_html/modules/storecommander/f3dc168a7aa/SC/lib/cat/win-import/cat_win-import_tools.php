	...
	*/
    ...
    function getReadableError() {
        ...
        if ($error['type'] === E_ERROR) {
            ...
            } else if(strpos($message,'Product->name length') !== false) {
                $return .= '<b>'._l('Product:name length > 128 characters').'</b>';
            } else if(strpos($message,'Product->name is empty') !== false) {
                $return .= '<b>'._l('Product:name is empty').'</b>';
            } else if(strpos($message,'Product->link_rewrite is not valid') !== false) {
                $return .= '<b>'._l('Product:link rewrite need only contains letters, numbers and following characters: _-').'</b>';
            } else if(strpos($message,'Product->link_rewrite is empty') !== false) {
                $return .= '<b>'._l('Product:link rewrite is empty').'</b>';
            } else if(strpos($message,'Product->link_rewrite length') !== false) {
                $return .= '<b>'._l('Product:link rewrite length > 128 characters').'</b>';
            } else if(strpos($message,'Product->ean13 is not valid') !== false) {
                $return .= '<b>'._l('Product:EAN13 is not valid').'</b><br/>';
            } else if(strpos($message,'Product->date_add is not valid') !== false) {
                $return .= '<b>'._l('Product:date add needs to be YYYY-MM-DD hh:mm:ss').'</b>';
            } else if(strpos($message,'Product->meta_description is not valid') !== false) {
                $return .= '<b>'._l('Product:meta description contains one or more invalid characters such as ^<>;=#{}').'</b>';
            } else if(strpos($message,'Product->meta_title is not valid') !== false) {
                $return .= '<b>'._l('Product:meta title contains one or more invalid characters such as ^<>;=#{}').'</b>';
            } else if(strpos($message,'Product->description is not valid') !== false) {
            ...
            } else if(strpos($message,'Product->price is empty') !== false) {
            ...
            } else if(strpos($message,'Combination->reference length') !== false) {
                $return .= '<b>'._l('Combination:reference length > 32 characters').'</b>';
            } else if(strpos($message,'FeatureValue->value is not valid') !== false) {
                $return .= '<b>'._l('Feature value:value contains one or more invalid characters such as ^<>;=#{}').'</b>';
            } else if(strpos($message,'Tag->name length') !== false) {
