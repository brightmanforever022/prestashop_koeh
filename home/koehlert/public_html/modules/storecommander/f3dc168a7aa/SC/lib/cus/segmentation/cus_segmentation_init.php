...
if (SCSG)
{
    ...
    function displaySegments(force_refresh, callback)
    {
        ...
        {
            $.post("index.php?ajax=1&act=cus_segmentation_relation_get&type=customer&idlist="+lastCustomerSelID+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function(data){
                selArray=data.split(',');
        ...
        else if(force_refresh==true)
        {
            ...
            prop_tb._segmentsGrid.loadXML("index.php?ajax=1&act=cus_segmentation_get&type=customers&id_lang="+SC_ID_LANG,function()
            {
