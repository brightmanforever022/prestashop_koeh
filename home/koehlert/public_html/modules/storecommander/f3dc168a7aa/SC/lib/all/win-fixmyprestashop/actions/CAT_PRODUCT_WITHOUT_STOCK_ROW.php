...
if(!empty($post_action) && $post_action=="do_check")
{
	...
	if(!empty($res) && count($res)>0)
	{
		...
		<script type="text/javascript">
			...
			tbProductWithoutStockRow.addButton("selectall", 0, "", "lib/img/application_lightning.png", "lib/img/application_lightning.png");
			tbProductWithoutStockRow.setItemToolTip('selectall','<?php echo _l('Select all')?>');
			tbProductWithoutStockRow.addButton("delete", 0, "", "lib/img/delete.gif", "lib/img/delete.gif");
			tbProductWithoutStockRow.setItemToolTip('delete','<?php echo _l('Delete associations')?>');
			tbProductWithoutStockRow.addButton("add", 0, "", "lib/img/add.png", "lib/img/add.png");
			...
			dhxlSCExtCheck.tabbar.cells("table_CAT_PRODUCT_WITHOUT_STOCK_ROW").attachStatusBar({text: '<?php echo _l('Count rows:')." ".count($res) ; ?>', height: 25 });
