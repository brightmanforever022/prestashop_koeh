	...
	switch($action){
		...
		case 'mapping_process':
			...
			if ((sc_in_array('priceinctax',$mappingData['DBArray'],"catWinImportProcess_DBArray") || sc_in_array('priceinctaxincecotax',$mappingData['DBArray'],"catWinImportProcess_DBArray")) && !sc_in_array('VAT',$mappingData['DBArray'],"catWinImportProcess_DBArray"))
					die(_l('Error in mapping: price including VAT found in CSV columns but no VAT column found. You need to indicate the VAT or use only price excluding VAT.'));
            ...
            if(sc_in_array('unit_price_ratio',$mappingData['DBArray'],"catWinImportCheck_DBArray") && sc_in_array('unit_price_ratio_ttc',$mappingData['DBArray'],"catWinImportCheck_DBArray")) {
                die(_l("!!! WARNING !!!")."</strong> "._l('Error in mapping: column unit price tax excl and unit price tax incl found. You need to choose only one of them.'));
            }
            ...
            if(sc_in_array('unit_price_ratio_ttc',$mappingData['DBArray'],"catWinImportCheck_DBArray") && !sc_in_array('VAT',$mappingData['DBArray'],"catWinImportCheckVAT_DBArray")) {
                die(_l("!!! WARNING !!!")."</strong> "._l('Error in mapping: column unit price tax incl found. You need to indicate the VAT or use only unit price excluding VAT.'));
            }
            ...
            if(sc_in_array('unit_price_impact',$mappingData['DBArray'],"catWinImportCheck_DBArray") && sc_in_array('unit_price_impact_ttc',$mappingData['DBArray'],"catWinImportCheck_DBArray")) {
                die(_l("!!! WARNING !!!")."</strong> "._l('Error in mapping: column unit price tax excl (combination impact) and unit price tax incl (combination impact) found. You need to choose only one of them.'));
            }
            ...
            if(sc_in_array('unit_price_impact_ttc',$mappingData['DBArray'],"catWinImportCheck_DBArray") && !sc_in_array('VAT',$mappingData['DBArray'],"catWinImportCheckVAT_DBArray")) {
                die(_l("!!! WARNING !!!")."</strong> "._l('Error in mapping: column unit price tax incl (combination impact) found. You need to indicate the VAT or use only unit price excluding VAT.'));
            }
				...
				if (sc_in_array('VAT',$mappingData['DBArray'],"catWinImportProcess_DBArray"))
				{
					...
					else
					{
						...
						if(!empty($vat_string) || (empty($vat_string) && $vat_string=="0") || !empty($vat_number))
						{
							...
							$sql="SELECT trg.name, trg.id_tax_rules_group,t.rate
								FROM `"._DB_PREFIX_."tax_rules_group` trg
								INNER JOIN `"._DB_PREFIX_."tax_rule` tr ON (trg.`id_tax_rules_group` = tr.`id_tax_rules_group`)
				...
				foreach($line AS $key => $value)
				{
					...
					if (sc_array_key_exists($key,$firstLineData) && sc_in_array($firstLineData[$key],$mappingData['CSVArray'],"catWinImportProcess_CSVArray"))
					{
						...
						switch($switchObject)
						{
                            ...
                            case 'additional_delivery_times':$newprod->additional_delivery_times = intval($value);break;
                            case 'delivery_in_stock':$newprod->delivery_in_stock[$id_lang] = psql($value);break;
                            case 'delivery_out_stock':$newprod->delivery_out_stock[$id_lang] = psql($value);break;
                            case 'low_stock_alert':$newprod->low_stock_alert = intval($value);break;
                            case 'low_stock_threshold':$newprod->low_stock_threshold = intval($value);break;
							case 'reference':
/*								if ($importConfig[$TODOfilename]['iffoundindb']=='replace'
							...
							case 'priceinctaxincecotax':
								$rate = $cache_VAT["rate"];
                                $getEcotaxLine = (float)findCSVLineValue('ecotax');
                            ...
                            case 'id_feature_value':
                                if ($value!='')
								{
                                    ...
                                    if(!empty($id_feature)) {
                                        ...
                                        $TODO[] = 'DELETE FROM ' . _DB_PREFIX_ . 'feature_product WHERE id_product = '.$prod_id.' AND id_feature = '.$id_feature;
                            ...
                            case 'ActionDeleteSpecificPrice':
                                if ($value!='' && $value!='0' && $newprod->id) {
                                    ...
                                    if(SCMS) {
                                        $sql .= ' AND `id_shop` IN ('.pSQL(implode(',',$id_shop_list)).')';
                                    }
                                    Db::getInstance()->Execute($sql);
                                }
                                break;
...
/* inutile pour le moment, on gère champ par champ
				...
				if ($newprod->save())
				{
					...
					if (_s('APP_COMPAT_HOOK') && !_s('APP_COMPAT_EBAY'))
						SCI::hookExec('updateProduct', array('id_product' => (int)$newprod->id,'product' => $newprod));
					if (_s('APP_DEBUG_CATALOG_IMPORT'))
					{
						$time_end = microtime(true);
						$time = $time_end - $time_start;
							...
							switch($switchObject)
							{
											...
											if (count($res)) {
                                                ...
                                                if(file_exists($attachment_import_path)) {
                                                    ...
                                                    if (copy($attachment_import_path, _PS_DOWNLOAD_DIR_.$uniqid)) {
                                                        ...
                                                        } elseif (version_compare(_PS_VERSION_,'1.4.0.3','>=')){
                                                            $sql = "INSERT INTO `"._DB_PREFIX_."attachment` (file,file_name,mime) VALUES ('".psql($uniqid)."','".psql($attachment)."','".psql($mime)."')";
                                                        }else{
                                                            $sql = "INSERT INTO `"._DB_PREFIX_."attachment` (file,mime) VALUES ('".psql($uniqid)."','".psql($mime)."')";
                                                        }
                                                        Db::getInstance()->Execute($sql);
								...
								case 'attribute_default_on':
									if (isCombinationWithID() && !empty($id_product_attribute) && getBoolean($value))
									{
										...
										{
											if($value) {
												$TODO[] = "UPDATE " . _DB_PREFIX_ . "product_attribute_shop SET default_on=".(version_compare(_PS_VERSION_,'1.6.1.0','>=') ? "NULL" : 0)." WHERE id_product = " . (int)$id_product . " AND id_shop IN (" . join(',',
														$id_shop_list) . ")";
								...
								case 'wholesale_price':
									...
									if(str_replace("supp_","",$id_supplier) == 'product' && !empty($newprod->id_supplier)) {
                                        ...
                                        if(!empty($supp_name) && array_key_exists($supp_name,$dataDB_supplierByName)) {
                                            $id_supplier = "supp_".$dataDB_supplierByName[$supp_name];
								...
								case 'specific_price_from_quantity':
									if ($value != '')
									{
										if ($newprod->id) {
                                            ...
                                            $specific_price_id_shop = explode(',',$specific_price_id_shop);
								...
								case 'ActionDeleteAllFeatures':
									if ($value!='' && $value!='0' && $newprod->id)
									{
                                        $sql = 'DELETE FROM `'._DB_PREFIX_.'feature_product` WHERE `id_product` = '.(int)$newprod->id;
                                        Db::getInstance()->Execute($sql);
									}
									break;
                                case 'feature_add':
                                    ...
                                    if ($values!='')
                                    {
                                        ...
                                        foreach($values_arr AS $value)
                                        {
                                            ...
                                            if ($id_feature && $id_feature_value)
                                            {
                                                $sql="DELETE FROM "._DB_PREFIX_."feature_product WHERE id_feature_value = ".intval($id_feature_value)." AND id_feature = ".intval($id_feature)." AND id_product = ".(int)$newprod->id;
                                                Db::getInstance()->Execute($sql);
                                                $sql="INSERT IGNORE INTO "._DB_PREFIX_."feature_product (id_feature_value,id_feature,id_product) VALUES (".intval($id_feature_value).",".intval($id_feature).",".(int)$newprod->id.")";
                                                Db::getInstance()->Execute($sql);
                                            }
                                            else
                                            {
                                ...
                                case 'feature_delete':
                                    ...
                                    if ($values!='')
                                    {
                                        ...
                                        foreach($values_arr AS $value)
                                        {
                                            ...
                                            if ($id_feature && $id_feature_value)
                                            {
                                                $sql="DELETE FROM "._DB_PREFIX_."feature_product WHERE id_feature=".intval($id_feature)." AND id_feature_value=".intval($id_feature_value)." AND id_product=".(int)$newprod->id;
                                                Db::getInstance()->Execute($sql);
                                            }
                                            /*else
                                            {
                                ...
                                case 'feature_custom':
                                    if ($value!='')
                                    {
                                        ...
                                        if ($id_feature > 0)
                                        {
                                            ...
                                            if($value=='-'){
                                                $sql="DELETE FROM "._DB_PREFIX_."feature_product WHERE id_feature=".intval($id_feature)." AND id_product='".(int)$newprod->id."'";
                                                Db::getInstance()->Execute($sql);
                                                $sql="DELETE FROM "._DB_PREFIX_."feature_value_lang WHERE id_feature_value IN (SELECT fv.id_feature_value FROM "._DB_PREFIX_."feature_value fv WHERE fv.id_feature_value=".intval($id_feature_value_OLD)." AND fv.custom=1)";
                                                Db::getInstance()->Execute($sql);
                                                $sql="DELETE FROM "._DB_PREFIX_."feature_value WHERE id_feature_value=".intval($id_feature_value_OLD)." AND custom=1";
                                                Db::getInstance()->Execute($sql);
                                            }else{
                                                $sql = "SELECT *
                                                          FROM "._DB_PREFIX_."feature_value fv
                                                            INNER JOIN "._DB_PREFIX_."feature_product fp ON (fv.id_feature=fp.id_feature AND fp.id_product='".(int)$newprod->id."')
                                                          WHERE fv.id_feature_value=".intval($id_feature_value_OLD)." AND fv.id_feature=".intval($id_feature);
                                                $fv=Db::getInstance()->getRow($sql);
                                                if ($fv['custom'])
                                                {
                                                    foreach($languages AS $lang){
                                                        $sql="UPDATE "._DB_PREFIX_."feature_value_lang SET value='".psql($value)."' WHERE id_feature_value=".intval($id_feature_value_OLD)." AND id_lang=".intval($lang['id_lang']);
                                                        Db::getInstance()->Execute($sql);
                                                    }
                                                }else{
                                                    $sql="DELETE FROM "._DB_PREFIX_."feature_product WHERE id_feature=".intval($id_feature)." AND id_product='".(int)$newprod->id."'";
                                                    ...
                                                    $sql="INSERT IGNORE INTO "._DB_PREFIX_."feature_product (id_feature_value,id_feature,id_product) VALUES (".intval($id_value).",".intval($id_feature).",'".(int)$newprod->id."')";
                                                    Db::getInstance()->Execute($sql);
                                                    foreach($languages AS $lang){
                                                        $sql="INSERT INTO "._DB_PREFIX_."feature_value_lang (id_feature_value,id_lang,value) VALUES (".intval($id_value).",".intval($lang['id_lang']).",'".psql($value)."')";
                                                        Db::getInstance()->Execute($sql);
                                                    }
                                                }
                                            }
...
//													$image->legend = createMultiLangField(str_replace('#','',Tools::substr($newprod->name[$defaultLanguageId],0,128)));
				...
				$final_sql = '';
				if(!empty($TODO_SET)) {
					$final_sql .= "UPDATE " . _DB_PREFIX_ . "product_attribute SET " . $todoSet_to_insert . " WHERE id_product_attribute=" . (int)$id_product_attribute . ";";
                }
                if(!empty($TODO_SET_SHOP)) {
                    $final_sql .= "UPDATE " . _DB_PREFIX_ . "product_attribute_shop SET " . $todoSetShop_to_insert . " WHERE id_product_attribute=" . (int)$id_product_attribute . " AND id_shop IN (" . psql(join(',', $id_shop_list)) . ")";
                }
                if(!empty($final_sql))
                {
          		...
          		if(!empty($image_id_array))
          		{
          			...
          			foreach($image_id_array as $actual_num=>$image_id)
          			{
                        ...
                        if(!empty($image_legend_array))
          				{
                            foreach($image_legend_array as $legend)
                            {
	          		...
	          		if (sc_array_key_exists($key,$firstLineData) && sc_in_array($firstLineData[$key],$mappingData['CSVArray'],"catWinImportProcess_CSVArray"))
	          		{
						...
						if(SCAS) {
	          			...
	          			switch($switchObject)
	          			{
							...
							break;
                            case 'low_stock_alert':
                                    $TODO="UPDATE "._DB_PREFIX_."product_attribute SET low_stock_alert=".pSQL($value)." WHERE id_product_attribute=".intval($id_product_attribute);
                                    Db::getInstance()->Execute($TODO);
                                    $TODO="UPDATE "._DB_PREFIX_."product_attribute_shop SET low_stock_alert=".pSQL($value)." WHERE id_product_attribute=".intval($id_product_attribute)." AND id_shop=".(int)$id_shop;
                                    Db::getInstance()->Execute($TODO);
                                break;
                            case 'low_stock_threshold':
                                    $TODO="UPDATE "._DB_PREFIX_."product_attribute SET low_stock_threshold=".pSQL($value)." WHERE id_product_attribute=".intval($id_product_attribute);
                                    Db::getInstance()->Execute($TODO);
                                    $TODO="UPDATE "._DB_PREFIX_."product_attribute_shop SET low_stock_threshold=".pSQL($value)." WHERE id_product_attribute=".intval($id_product_attribute)." AND id_shop=".(int)$id_shop;
                                    Db::getInstance()->Execute($TODO);
                                break;
                            case 'image_id':
                                if(!empty($value))
