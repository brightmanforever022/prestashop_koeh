...
{
	public $name="Customers in a specific department";
	...
	public function _executeHook_segmentAutoConfig($name, $params=array())
	{
		...
		$html.='</select>
		<br/><br/>		
		<strong>'._l("Department:").'</strong><br/>
		<select id="id_state" name="id_state" style="width: 100%;"></select>
		...
		{
			$html.='$.post("index.php?ajax=1&act=all_win-segmentation_gate&id_lang='.$params["id_lang"].'",{"segment":"customersInSpecificDepartmentSegment", "function":"_getAttributesForIdCountry", "params": {"id_country":"'.intval($values['id_country']).'"}},function(data){
					$("#id_state").html(data);
					$("#id_state").val('.intval($values['id_state']).');
				});';
		}
		...
		if(!empty($params["auto_params"]))
		{
			...
			{			
				...
				FROM "._DB_PREFIX_."customer c
					INNER JOIN "._DB_PREFIX_."address a ON (c.id_customer = a.id_customer AND a.id_state='".intval($auto_params["id_state"])."')
				WHERE a.deleted!=0";
			...
			elseif(!empty($auto_params["id_country"]))
			{
				$sql = "SELECT DISTINCT(c.id_customer)
				FROM "._DB_PREFIX_."customer c
					INNER JOIN "._DB_PREFIX_."address a ON (c.id_customer = a.id_customer AND a.id_country='".intval($auto_params["id_country"])."')
				WHERE a.deleted!=0";
				$res=Db::getInstance()->ExecuteS($sql);
				foreach($res AS $row)
				{
	...
	public function _executeHook_segmentAutoSqlQuery($name, $params=array())
	{
		...
		if(!empty($params["auto_params"]))
		{
			...
			if(!empty($auto_params["id_state"]))
			{
				...
				{
					$where = " AND c.id_customer IN (SELECT DISTINCT(cu.id_customer)
													FROM "._DB_PREFIX_."customer cu
														INNER JOIN "._DB_PREFIX_."address a ON (cu.id_customer = a.id_customer AND a.id_state='".intval($auto_params["id_state"])."')
													WHERE a.deleted!=0
													)";
				}
				else
				{
					$where = " AND a.id_state='".intval($auto_params["id_state"])."'";
				}
			...
			elseif(!empty($auto_params["id_country"]))
			{
				if(!empty($params["is_customer"]))
				{
					$where = " AND c.id_customer IN (SELECT DISTINCT(cu.id_customer)
													FROM "._DB_PREFIX_."customer cu
														INNER JOIN "._DB_PREFIX_."address a ON (cu.id_customer = a.id_customer AND a.id_country='".intval($auto_params["id_country"])."')
													WHERE a.deleted!=0
													)";
				}
				else
				{
					$where = " AND a.id_country='".intval($auto_params["id_country"])."'";
				}
			}
		}
		return $where;
	...
	static public function _getAttributesForIdCountry($params=array())
	{
