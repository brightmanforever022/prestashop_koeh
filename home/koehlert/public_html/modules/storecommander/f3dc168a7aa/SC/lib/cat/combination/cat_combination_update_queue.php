			...
			if(!empty($log_ids[$num]))
			{
				...
				elseif(!empty($action) && $action=="update" && !empty($gr_id))
				{
					...
					if(!empty($id_product))
					{
						...
						if ((isset($_POST['quantityupdate']) || isset($_POST['quantity'])) && ($updated_field=="quantityupdate" || $updated_field=="quantity"))
						{
                            ...
                            if (version_compare(_PS_VERSION_, '1.7.2.0', '>='))
                            {
                                ...
                                if(SCMS)
                                {
                                    ...
                                    $old_rows[] = Db::getInstance()->getRow("SELECT * FROM " . _DB_PREFIX_ . "stock_available WHERE id_product='" . (int)$id_product . "' AND id_product_attribute='".(int)$id_product_attribute."'");
                            }
						...
						{
							if (_s('APP_COMPAT_HOOK'))
								SCI::hookExec('updateProductAttribute', array('id_product_attribute' => (int)$id_product_attribute,'product'=>new Product(intval($id_product))));
						}elseif(_s('APP_COMPAT_EBAY')){
							Configuration::updateValue('EBAY_SYNC_LAST_PRODUCT', min(Configuration::get('EBAY_SYNC_LAST_PRODUCT'),intval($id_product)));
						}
