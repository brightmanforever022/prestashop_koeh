...
if(!empty($post_action) && $post_action=="do_check")
{
	...
	if(hasUpdated($res))
	{
		...
		<script type="text/javascript">
			...
			dhxlSCExtCheck.tabbar.cells("table_CAT_PROD_NOT_SAME_ATTRIBUTES").attachStatusBar({text: '<?php echo _l('Count rows:')." ".count($res) ; ?>', height: 25 });
			...
			tbRightNotSameAttrProduct.addButton("delete", 0, "", "lib/img/delete.gif", "lib/img/delete.gif");
			tbRightNotSameAttrProduct.setItemToolTip('delete','<?php echo _l('Delete')?>');
			tbRightNotSameAttrProduct.addButton("save", 0, "", "lib/img/disk.png", "lib/img/disk.png");
						...
						WHERE id_product_attribute='".(int)$row["id_product_attribute"]."' 
							AND id_attribute IN (
								...
								WHERE a.`id_attribute` = `"._DB_PREFIX_."product_attribute_combination`.`id_attribute`
									AND a.`id_attribute_group` IN (".pSQL(implode(",", $to_delete)).")
							)";
				...
				foreach($to_insert as $id)
				{
					if(empty($default_values[$id]))
					{
						$sql = "SELECT `id_attribute`
							...
							WHERE `id_attribute_group` = '".intval($id)."'
							ORDER BY position
							LIMIT 1";
