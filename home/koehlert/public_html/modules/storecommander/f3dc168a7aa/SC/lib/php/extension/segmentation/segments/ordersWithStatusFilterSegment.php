...
class ordersWithStatusFilterSegment extends SegmentCustom
{
    ...
    public function _executeHook_segmentAutoConfig($name, $params=array())
    {
        ...
        <div><input type="checkbox" name="use_filters" '.($values['use_filters'] == 1 ? 'checked=checked' : '').'>'._l("Use order filters").'</div>';

        $html .='<strong>'._l("Searched status:").'</strong><br/>
		<select id="id_searched_status" name="id_searched_status" style="width: 100%;" multiple="multiple">
			<option value="">--</option>';


        $id_searched_status = explode(',',$values ['id_searched_status']);
        $sql="SELECT DISTINCT os.id_order_state,os.name FROM "._DB_PREFIX_."order_state_lang AS os, "._DB_PREFIX_."order_history AS oh WHERE oh.id_order_state = os.id_order_state AND os.id_lang = ".intval($id_lang)." ORDER BY os.name";
		...
		<select id="id_include_status" name="id_include_status" style="width: 100%" multiple="multiple">
			<option value="">--</option>';
		...
		<select id="id_exclude_status" name="id_exclude_status" style="width: 100%" multiple="multiple">
			<option value="">--</option>';
    ...
    public function _executeHook_segmentAutoSqlQueryGrid($name, $params=array())
    {
        ...
        if(!empty($params["auto_params"]))
        {
            ...
            if((!empty($auto_params["id_searched_status"])) || (!empty($auto_params["id_include_status"])) || (!empty($auto_params["id_exclude_status"])))
            {
                ...
                if(!empty($auto_params["id_searched_status"]))
                        $sql.= " AND oh.id_order_state IN ( " . pSQL($auto_params["id_searched_status"]) . ") ";
                if(!empty($auto_params["id_include_status"]))
                    $sql.= " AND o.current_state IN (" . pSQL($auto_params["id_include_status"]) . ") ";
                if(!empty($auto_params["id_exclude_status"]))
                    $sql.= " AND o.current_state NOT IN (" . pSQL($auto_params["id_exclude_status"]) . ") ";
    ...
    public function _executeHook_segmentAutoSqlQuery($name, $params=array())
    {
        ...
        if(!empty($params["auto_params"]))
        {
            ...
            if((!empty($auto_params["id_searched_status"])) || (!empty($auto_params["id_include_status"])) || (!empty($auto_params["id_exclude_status"])))
            {
                ...
                {
                    $where = " AND o.id_order IN (".$sql.")";
                }
                else
                {
                    $where = " AND od.id_order IN (".$sql.")";
                }
            }
        }
        return $where;
