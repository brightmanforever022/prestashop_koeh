...
function getRowsFromDB(){
    ...
    foreach ($res AS $history)
    {
        $xml.=("<row id='".$history['id_order_detail']."'>");
            ...
            if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
            {
                $xml .= ("<cell>" . number_format($history['unit_price_tax_excl'], 2, '.', '') . "</cell>");
            }
...
<head>
<beforeInit>
<call command="attachHeader"><param><![CDATA[#numeric_filter,#numeric_filter,#numeric_filter,#numeric_filter,#text_filter,#numeric_filter,<?php if (version_compare(_PS_VERSION_, '1.2.0.0', '>=')){ ?>#numeric_filter,#numeric_filter,<?php } ?>#numeric_filter,#numeric_filter<?php if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')){ ?>,#numeric_filter<?php }?>,#text_filter<?php if (version_compare(_PS_VERSION_, '1.4.0.0', '>=')){ ?>,#text_filter<?php } ?>,#text_filter,#text_filter,#numeric_filter,#numeric_filter,#numeric_filter]]></param></call>
<call command="attachFooter"><param><![CDATA[,,,,,<?php if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')){ ?>,<?php } ?><?php if (version_compare(_PS_VERSION_, '1.2.0.0', '>=')){ ?>,,<?php } ?>,,#stat_total]]></param></call>
</beforeInit>
