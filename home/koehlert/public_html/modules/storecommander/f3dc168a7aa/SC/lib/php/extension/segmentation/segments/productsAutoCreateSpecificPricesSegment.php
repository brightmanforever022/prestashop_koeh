...
class productsAutoCreateSpecificPricesSegment extends SegmentCustom
{
	...
	public function _executeHook_segmentAutoConfig($name, $params=array())
	{
		...
		$(document).ready(function(){
			$("#reset_processed").click(function(){
				$.post("index.php?ajax=1&act=all_win-segmentation_gate&id_lang='.$params["id_lang"].'",{"segment":"productsAutoCreateSpecificPricesSegment", "function":"_resetProcessed", "params": {"id_segment":"'.intval($params["id_segment"]).'"}},function(data){
	...
	public function _executeHook_segmentAutoSqlQueryGrid($name, $params=array())
	{
		...
		if(!empty($params["id_segment"]))
		{
			$sql = "SELECT *
				...
				WHERE id_segment ='".intval($params["id_segment"])."'";
			...
			foreach($res AS $row)
			{
				if($row["type_element"]=="product")
				{
					...
					else
					{
						...
						WHERE id_product = '".intval($row['id_element'])."'";
					...
					$array[] = array($type, $name, $infos, "id"=>$row["type_element"]."_".$row['id_element']);		
				}
			}
		}
		return $array;
	...
	public function _executeHook_segmentAutoSqlQuery($name, $params=array())
	{
		...
		{
			$where = " AND p.id_product IN (SELECT id_element FROM "._DB_PREFIX_."sc_segment_element WHERE type_element='product' AND id_segment='".intval($params["id_segment"])."')";
		}
		return $where;
	}
	...
	public function _executeHook_cronSegment($name, $params=array())
	{
		...
		{
			$seg_params = unserialize($segment['auto_params']);
			$seg_params["nb_days"] = intval($seg_params["nb_days"]);
				...
				&& !empty($seg_params["playing_time"]) && preg_match("/(2[0-3]|[01][0-9]):[0-5][0-9]/", $seg_params["playing_time"]))
			{
				...
				if(($hour_now>=$seg_params["playing_time"]) && (empty($seg_params["last_cron"]) || $seg_params["last_cron"]<$playing_time))
				{
					...
					$sql = "SELECT p.id_product
						...
						WHERE sc_se.id_segment ='".intval($segment["id_segment"])."'
							AND sc_se.type_element='product'
							AND p.date_add <= (SELECT ADDDATE('".date("Y-m-d")." 00:00:00', INTERVAL -".intval($seg_params["nb_days"])." DAY))
							".(!empty($already)?" AND p.id_product NOT IN ('".$already."') ":"")."
					...
					foreach($products as $product)
					{
						...
						{
							$specificPrice = new SpecificPrice(intval($exist[0]["id_specific_price"]));
							$specificPrice->reduction = (float)(floatval($seg_params["percent"]) / 100);
							$specificPrice->save();
					...
					$sql = "UPDATE "._DB_PREFIX_."sc_segment SET auto_params = '".$seg_params_seria."' WHERE id_segment='".intval($segment["id_segment"])."'";
	...
	static public function _resetProcessed($params=array())
	{
		if(!empty($params["id_segment"]))
		{
			$sql = "SELECT *
			...
			$seg_params["already"] = "";
			$seg_params_seria = serialize($seg_params);
			$sql = "UPDATE "._DB_PREFIX_."sc_segment SET auto_params = '".$seg_params_seria."' WHERE id_segment='".intval($params["id_segment"])."'";
			Db::getInstance()->Execute($sql);
		}
	}
}