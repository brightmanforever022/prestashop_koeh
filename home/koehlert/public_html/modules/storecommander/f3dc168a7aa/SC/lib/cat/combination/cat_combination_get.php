 ...
 **/
    ...
    if(!empty($reference_check)) {
        ...
        if(!empty($res)) {
            foreach($res as $row) {
                $error .= _l('Dupliace reference found with product ID:').' '.$row['id_product'].'<br/>';
            }
        }
        ...
        if(!empty($res)) {
            foreach($res as $row) {
                $error .= _l('Dupliace reference found with combination ID:').' '.$row['id_product_attribute'].'<br/>';
            }
        }
        if(!empty($error)) {
            die($error);
	...
	function getRowsFromDB()
	{
		...
		if(!empty($sortByPosition)) {
            usort($combArray, "comparePosition");
        }
		...
		foreach ($combArray AS $id_product_attribute => $product_attribute)
		{
			...
			if(version_compare(_PS_VERSION_, '1.6.0.0', '>=') || (version_compare(_PS_VERSION_, '1.3.0.0', '>=') && ($product_attribute['ecotax']*1)==0))
...
/*	function getColIndex($col)
	...
	function getColSettingsAsXML()
	{
		...
		$sql = '
		SELECT pa.id_product_attribute,pa.id_product '.(!empty($sortByPosition) ? ',a.`position`': '').((!empty($pa_cols))?',pa.`'.implode("`,pa.`",$pa_cols):'').'`, ag.`id_attribute_group`, agl.`name` AS group_name, al.`name` AS attribute_name, a.`id_attribute` '.(version_compare(_PS_VERSION_, '1.5.0.1', '>=')?', a.position ':'').', p.`price` AS pprice, p.`weight` AS pweight, p.`ecotax` AS pecotax'.
		...
		if(version_compare(_PS_VERSION_, '1.7.5.0', '>=')) {
		    $sql = str_replace('pa.`location_new`', 'sa.`location` as location_new', $sql);
        }
...
*/
		...
		{
		    if(!empty($sortByPosition)) {
		       $combArray[$combinaison['id_product_attribute']]['position'] = (int)$combinaison['position'];
            }
			...
			if(version_compare(_PS_VERSION_, '1.7.5.0', '>=')) {
                $combArray[$combinaison['id_product_attribute']]['location_new'] = $combinaison['location_new'];
            }
