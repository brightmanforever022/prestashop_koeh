...
//  		prop_tb._accessoriesGrid.enableSmartRendering(true);
			...
			// dynamic filter
            prop_tb._accessoriesGrid.attachEvent("onFilterEnd", function(elements)
            {
                ...
                if(nb_cols>0)
                {
                    for(var i=0; i<nb_cols; i++)
                    {
                        ...
                        if(prop_tb._accessoriesGrid.getFilterElement(i)!=null && (colId =="reference" || colId =="name"))
                        {
                            ...
                            if((colValue!=null && colValue!="") || (oldFilters[i]!=null && oldFilters[i]!=""))
                            {
                                ...
                                oldFilters[i] = prop_tb._accessoriesGrid.getFilterElement(i).value;
                            }
                        }
                    }
                }
...
function displayAccessories(callback,forceAllProducts)
{
	if (prop_tb._accessoriesGrid._rowsNum > 0)
	{
		...
		prop_tb._accessoriesGrid.clearAll(true);
		prop_tb._accessoriesGrid.load("index.php?ajax=1&act=cat_accessory_get&filter_params="+filter_params+(cat_grid.getSelectedRowId()!=null?"&idlist="+cat_grid.getSelectedRowId():"")+(display_all_products==1?"&forceAllProducts=1":"")+"&accessory_filter="+accessoriesFilter+"&id_category="+catselection+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function(){
				...
				filter_params ="";
                 ...
                 if(prop_tb._accessoriesGrid._first_loading == 0) {
                    oldFilters.forEach(function(value,index){
                        prop_tb._accessoriesGrid.getFilterElement(index).value=value;
                        ...
                        $(prop_tb._accessoriesGrid.getFilterElement(index)).keyup(function(){
                            ...
                            if(input_value == ""){
                                ...
                                oldFilters.forEach(function(value,index){
                                    if(filter_params!="")
                                        filter_params = filter_params + ",";
                                    var colname=prop_tb._accessoriesGrid.getColumnId(index);
                                    filter_params = filter_params + colname+"|||"+value;
                                });
                                prop_tb._accessoriesGrid._rowsNum = 0;
                                displayAccessories();
                            }
