 ...
 **/
	...
	{
		global $idlist,$id_lang,$forceAllProducts,$used,$accessory_filter,$id_category,$filter_params;
		if($forceAllProducts==0)
		{
			...
			if(!empty($filter_params)) {
                ...
                $condition = (!empty($accessory_filter) ? 'AND' : 'WHERE');
				foreach ($filters as $filter)
				{
					...
					if(!empty($field) && !empty($search)) {
                        switch($field){
                            case 'name':
                                $sql_filter[]= "pl.`".$field."` LIKE '%".pSQL($search)."%'";
                                break;
                            default:
                                $sql_filter[]= "p.`".$field."` LIKE '%".pSQL($search)."%'";
                                break;
                        }
					}
				}
		...
		foreach($res as $row){
            ...
            if(_s('CAT_PROPERTIES_ACCESSORY_IMAGE')) {
                ...
                {
                    if (file_exists(SC_PS_PATH_REL."img/p/".getImgPath((int)$row['id_product'],(int)$row['id_image'],_s('CAT_PROD_GRID_IMAGE_SIZE')))) {
                        echo "<cell><![CDATA[<img src='".SC_PS_PATH_REL."img/p/".getImgPath((int)$row['id_product'],(int)$row['id_image'],_s('CAT_PROD_GRID_IMAGE_SIZE'))."'/>]]></cell>";
                    } else {
...
<head>
<beforeInit>
<call command="attachHeader"><param><![CDATA[#text_filter,,#select_filter,<?php (_s('CAT_PROPERTIES_ACCESSORY_IMAGE') ? ',' : '')?>#text_filter,#text_filter,#text_filter,#text_filter]]></param></call>
</beforeInit>
