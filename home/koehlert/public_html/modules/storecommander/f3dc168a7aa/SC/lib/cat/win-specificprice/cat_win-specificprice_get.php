...
function generateValue($col, $row, $p, $pa=null, $params=array(), $row_color = null)
{
	...
	switch($col){
		...
		case 'price_wt_with_reduction':
			$return .= "<cell".(!empty($row_color) ? ' style="'.$row_color.'"' : '')."><![CDATA[".number_format($params['price_wt_with_reduction'], 2, '.', '')."]]></cell>";
			break;
		case 'price_it_with_reduction':
			$return .= "<cell".(!empty($row_color) ? ' style="'.$row_color.'"' : '')."><![CDATA[".number_format($params['price_it_with_reduction'], 2, '.', '')."]]></cell>";
			break;
		case 'shop_id':
