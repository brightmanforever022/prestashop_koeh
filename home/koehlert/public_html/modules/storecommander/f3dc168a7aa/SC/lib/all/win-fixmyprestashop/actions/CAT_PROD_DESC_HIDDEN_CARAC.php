...
if(!empty($post_action) && $post_action=="do_check")
{
	...
	if(!empty($res) && count($res)>0)
	{
		...
		<script type="text/javascript">
			/*var win_width = $(window).width()-500;
			if(win_width>0)
			{
				toolsCheckWindow.setDimension(win_width);
				dhxlSCExtCheck.cells('b').setWidth(400);
			}*/
		
			var tbHiddenCarac = dhxlSCExtCheck.tabbar.cells("table_CAT_PROD_DESC_HIDDEN_CARAC").attachToolbar();
			tbHiddenCarac.addButton("selectall", 0, "", "lib/img/application_lightning.png", "lib/img/application_lightning.png");
			tbHiddenCarac.setItemToolTip('selectall','<?php echo _l('Select all')?>');
			tbHiddenCarac.addButton("replace", 0, "", "lib/img/database_refresh.png", "lib/img/database_refresh.png");
			...
			gridHiddenCarac.setHeader("ID,<?php echo _l('Reference')?>,<?php echo _l('Name')?>,<?php echo _l('Lang')?><?php if(version_compare(_PS_VERSION_, '1.5.0.0', '>=')) echo ","._l('Shop') ?>");
			gridHiddenCarac.setInitWidths("100, 80, 110<?php if(version_compare(_PS_VERSION_, '1.5.0.0', '>=')) echo", 100"; ?>,100");
			gridHiddenCarac.setColAlign("left,left,left<?php if(version_compare(_PS_VERSION_, '1.5.0.0', '>=')) echo",left"; ?>,left");
			...
			dhxlSCExtCheck.tabbar.cells("table_CAT_PROD_DESC_HIDDEN_CARAC").attachStatusBar({text: '<?php echo _l('Count rows:')." ".count($res) ; ?>', height: 25 });
