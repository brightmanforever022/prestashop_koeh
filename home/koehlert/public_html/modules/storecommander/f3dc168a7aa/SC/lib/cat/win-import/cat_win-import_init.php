...
// IMPORT
	lastCSVFile='';
	mapping='';
	arrayFieldLang=Array('name','description','description_short','meta_title','meta_description','meta_keywords','link_rewrite','available_now','available_later','tags','customization_field_name','image_legend','delivery_in_stock','delivery_out_stock'<?php echo sc_ext::readImportCSVConfigXML('definitionForLangField');?>);
	arrayFieldOption=Array('feature','id_feature_value','feature_custom','attribute','attribute_multiple'<?php if(SCAS) echo ",'quantity','location','add_quantity','remove_quantity','quantity_on_sale'"; if(version_compare(_PS_VERSION_, '1.5.0.0', '>=')) echo ",'supplier_reference','wholesale_price','supplier_currency'"; echo sc_ext::readImportCSVConfigXML('definitionForOptionField');?>);
	var comboArray = null;
	...
	wImport.tbOptions.addButton("go_to_settings", 0, "", "lib/img/cog_edit.png", "lib/img/cog_edit.png");
    wImport.tbOptions.setItemToolTip('go_to_settings','<?php echo _l('Open settings')?>');
    ...
    wImport.tbOptions.addInput("filter_name", 0,"",100);
    wImport.tbOptions.setItemToolTip('filter_name','<?php echo _l('Filter by name')?>');
    wImport.tbOptions.addText('txt_filter_name', 0, '<?php echo _l('Filter by name')?>');
		...
		wImport.gridMapping.forEachRow(function(rId){
			if (wImport.gridMapping.cells(rId,0).getValue()=="1")
			{
				...
				if (wImport.gridMapping.cells(rId,idxDBField).getValue()=='feature'
							&& wImport.gridMapping.cells(rId,idxOptions).getValue()=='')
					flag=false;
				if (wImport.gridMapping.cells(rId,idxDBField).getValue()=='id_feature_value'
							&& wImport.gridMapping.cells(rId,idxOptions).getValue()=='')
					flag=false;
...
?>
	...
	wImport.tbProcess.setItemToolTip('check','<?php echo _l('Votre import est-il pr�t ?')?>');
...
<?php
    ...
    if (version_compare(_PS_VERSION_, '1.7.3.0', '>='))
    {
        ...
        $array[_l('low stock alert',1)]="comboDBField.put('low_stock_alert','"._l('low stock alert',1)."');";
        $array[_l('low stock threshold',1)]="comboDBField.put('low_stock_threshold','"._l('low stock threshold',1)."');";
    }
    if ( version_compare(_PS_VERSION_, '1.7.3.0', '>=') || (
