	...
	if(!empty($_GET["ids"]))
	{
		...
		if(!empty($ids))
		{
			$sql = "SELECT *
							...
							WHERE scaff_partner_id IN (".psql($ids).")
...
<head>
<beforeInit>
<call command="attachHeader"><param><![CDATA[<?php if($multiple) echo '#text_filter,'; ?>#numeric_filter<?php if(SCMS) { ?>,#select_filter<?php } ?>,#text_filter,#text_filter,#text_filter,#text_filter,#select_filter,#select_filter,#text_filter]]></param></call>
</beforeInit>
<?php if($multiple) { ?>
...
<column id="scaff_partner_mode" width="80" type="coro" align="right"><?php echo _l('Mode')?>
