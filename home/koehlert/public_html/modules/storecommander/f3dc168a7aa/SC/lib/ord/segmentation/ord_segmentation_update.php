...
$action=intval(Tools::getValue('action'));
$type = Tools::getValue('type');
$id_element=intval(Tools::getValue('id_element'));
$value=(Tools::getValue('value'));

if($action=="present")
{
	$ids=(Tools::getValue('ids'));
	$id_segment=intval(Tools::getValue('id_segment'));
	if(!empty($id_segment) && !empty($type) && !empty($ids))
	{
			...
			foreach($ids as $id_element)
			{
				if(!ScSegmentElement::checkInSegment($id_segment, $id_element, $type))
				{
					...
					if($manual_add)
					{
						...
						$segment_element->id_element = intval($id_element);
		...
		else
		{
			...
			foreach($ids as $id_element)
			{
				...
				WHERE id_segment ='".intval($id_segment)."' AND type_element='".pSQL($type)."' AND id_element ='".intval($id_element)."'";
