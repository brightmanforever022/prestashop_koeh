<?php
	...
	$id_banner=intval(Tools::getValue('gr_id',0));
	$action=(Tools::getValue('action',""));
	$value=intval(Tools::getValue('value',0));
	...
	if(isset($_POST["!nativeeditor_status"]) && trim($_POST["!nativeeditor_status"])=="updated"){
		if(!empty($id_banner))
		{
			...
			if (count($todo))
			{
				$sql = "UPDATE "._DB_PREFIX_."scaff_banner SET ".join(' , ',$todo)." WHERE id_banner=".intval($id_banner);
		...
		if(!empty($id_banner))
		{
			...
			/*$sql = "DELETE FROM "._DB_PREFIX_."scaff_banner
					WHERE id_banner=".intval($id_banner);
			Db::getInstance()->Execute($sql);*/
		}
		$newId = $_POST["gr_id"];
		$action = "delete";
	...
	elseif($action=="mass_active")
	{
		if(!empty($id_banners))
		{
			$sql = "UPDATE "._DB_PREFIX_."scaff_banner SET active='".(int)$value."' WHERE id_banner IN (".pSQL($id_banners).")";
			Db::getInstance()->Execute($sql);
		}
	}
	elseif($action=="url")
	{
		...
		if(!empty($id_banner))
		{
			$sql = "UPDATE "._DB_PREFIX_."scaff_banner SET url='".pSQL(urldecode($url))."' WHERE id_banner = '".intval($id_banner)."'";
			Db::getInstance()->Execute($sql);
		}
	}
