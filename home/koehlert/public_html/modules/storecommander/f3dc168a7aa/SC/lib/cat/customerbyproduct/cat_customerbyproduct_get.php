 ...
 **/
	...
	$sql = 'SELECT o.id_order,od.id_order_detail, o.date_add, c.firstname, c.lastname, c.id_customer, c.email, c.newsletter,'.(version_compare(_PS_VERSION_, '1.5.4.0', '>=') ? ' c.id_lang as cus_lang,' : '').' o.payment, od.product_name, od.product_id, od.product_attribute_id, od.product_quantity';
	...
	foreach($res AS $row)
	{
	        //$unique = implode('_',array($row['id_customer'],$row["product_id"],$row["product_attribute_id"]));
			$lang = $language_arr[$row['cus_lang']];
			$xml.=("<row id='".$row['id_order_detail']."'>");
                ...
                $xml.=("<cell><![CDATA[".$row['newsletter']."]]></cell>");
				//if($multiple) {
					$id = $row["product_id"];
...
<head>
<beforeInit>
    <call command="attachHeader"><param><![CDATA[#numeric_filter,#numeric_filter,<?php if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')) { ?>#text_filter,<?php }?>#text_filter,#text_filter,#text_filter,<?php if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')) { ?>#text_filter,<?php }?>#numeric_filter,#numeric_filter,#text_filter,#numeric_filter,#select_filter,#text_filter,#text_filter]]></param></call>
    <call command="attachFooter"><param><![CDATA[,,<?php if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')) { ?>,<?php }?>,,,<?php if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')) { ?>,<?php }?>,,,#stat_total,,,]]></param></call>
</beforeInit>
...
<column id="newsletter" width="150" type="coro" align="left" sort="str"><?php echo _l('Newsletter')?>
