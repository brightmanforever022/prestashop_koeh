...
if($action!='' && !empty($id_supplier) && !empty($idlist)/* && !empty($id_actual_supplier)*/)
{
	...
	if(!empty($idlist)) {
		...
		//update date_upd
		$sql = "UPDATE "._DB_PREFIX_."product SET date_upd = '".pSQL(date("Y-m-d H:i:s"))."' WHERE id_product IN (".pSQL($idlist).");";
		if(SCMS) {
			$sql .= "UPDATE "._DB_PREFIX_."product_shop SET date_upd = '".pSQL(date("Y-m-d H:i:s"))."' WHERE id_product IN (".pSQL($idlist).") AND id_shop IN (".pSQL(SCI::getSelectedShopActionList(true)).")";
		}
		Db::getInstance()->Execute($sql);
		// PM Cache
