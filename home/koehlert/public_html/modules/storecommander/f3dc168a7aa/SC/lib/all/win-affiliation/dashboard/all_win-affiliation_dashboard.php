...
if(SCMS)
{
	$shop = intval(Tools::getValue("id_shop", "0"));
...
$(document).ready(function(){
	...
	$(".btn.submit").click(function(){
        $.post("index.php?ajax=1&act=all_win-affiliation_dashboard_get&id_lang="+SC_ID_LANG<?php if(!empty($id_shop)) echo '+"&id_shop='.$id_shop.'"' ?>, { "start_month": $("#start_month").val(),"start_year": $("#start_year").val(),"end_month": $("#end_month").val(),"end_year": $("#end_year").val()}, function(data){
	...
	$("#export").click(function(){
        window.open("index.php?ajax=1&act=all_win-affiliation_dashboard_export&id_lang="+SC_ID_LANG+"&start_month="+$("#start_month").val()+"&start_year="+$("#start_year").val()+"&end_month="+$("#end_month").val()+"&end_year="+$("#end_year").val()<?php if(!empty($id_shop)) echo '+"&id_shop='.$id_shop.'"' ?>);
    });
});
</script>
</head>
	...
	<div class="col_left-in">
		...
		<div class="input_bloc">
			...
			<select id="start_month" class="month" default="<?php echo $now*1; ?>">
				<option value="1" <?php if($now=="01") echo "selected"; ?>><?php echo _l('January');?> (01)</option>
				<option value="2" <?php if($now=="02") echo "selected"; ?>><?php echo _l('February');?> (02)</option>
				<option value="3" <?php if($now=="03") echo "selected"; ?>><?php echo _l('March');?> (03)</option>
				<option value="4" <?php if($now=="04") echo "selected"; ?>><?php echo _l('April');?> (04)</option>
				<option value="5" <?php if($now=="05") echo "selected"; ?>><?php echo _l('May');?> (05)</option>
				<option value="6" <?php if($now=="06") echo "selected"; ?>><?php echo _l('June');?> (06)</option>
				<option value="7" <?php if($now=="07") echo "selected"; ?>><?php echo _l('July');?> (07)</option>
				<option value="8" <?php if($now=="08") echo "selected"; ?>><?php echo _l('August');?> (08)</option>
				<option value="9" <?php if($now=="09") echo "selected"; ?>><?php echo _l('September');?> (09)</option>
				<option value="10" <?php if($now=="10") echo "selected"; ?>><?php echo _l('October');?> (10)</option>
				<option value="11" <?php if($now=="11") echo "selected"; ?>><?php echo _l('November');?> (11)</option>
				<option value="12" <?php if($now=="12") echo "selected"; ?>><?php echo _l('December');?> (12)</option>
			</select>
			<?php 
			$now = date("Y");
			$start = $now - 10;
			...
			<select id="start_year" class="year" default="<?php echo $now_moins; ?>">
				<?php
				for ($i=$start; $i<=$now; $i++) { ?>
					<option value="<?php echo $i; ?>" <?php if($now_moins==$i) echo "selected"; ?>><?php echo $i; ?></option>
				<?php } ?>
			</select>
		</div>
...
<div class="col_right">
	...
	<table class="table" width="100%">
		...
		?>
		<tr>
			<td class="aright"><?php echo number_format($current_amount,2); ?></td>
			<td class="aright"><?php echo $click; ?></td>
			<td class="aright"><?php echo $visiteur; ?></td>
			<td class="aright"><?php echo $nb_cmd; ?></td>
			<td class="aright"><?php if(!empty($visiteur)) echo number_format($nb_cmd/$visiteur*100); else echo "0"; ?>%</td>
		</tr>		
	</table>

	<br/><br/>
	<h3><?php echo _l('Period');?></h3>
	<img id="export" src="lib/img/page_excel.png" alt="<?php echo _l('Export');?>" title="<?php echo _l('Export');?>" style="float: right; cursor: pointer; margin-top: -20px;" />
	<table class="table" width="100%">
		...
		<?php 
		list($start_month,$start_year) = explode("/",$start_period);
		...
		for($y=$start_year; $y<=$end_year; $y++)
		{
			for($m=1; $m<=12; $m++)
			{
				...
				if($add)
				{
					...
					$visiteur = SCAffClick::GetNbVisiteurByDate($day_start, $day_end, null, $id_shop);
					?>
					<tr class="deletable <?php if($i>0 && ($i%2)) echo "edd";?>">
