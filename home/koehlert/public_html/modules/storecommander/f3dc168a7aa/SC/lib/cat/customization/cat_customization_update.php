...
if(Tools::getValue('act','')=='cat_customization_update')
{
	if ($action=='delete')
	{
        ...
        {
            $exp = explode(",",$customization_list);
            foreach ($exp as $customization)
            {
                $sql="SELECT cf.*
				FROM "._DB_PREFIX_."customization c
				    INNER JOIN "._DB_PREFIX_."customized_data cd ON (c.id_customization=cd.id_customization)
				        INNER JOIN "._DB_PREFIX_."customization_field cf ON (cf.id_customization_field=cd.index AND cf.type=cd.type AND cf.id_customization_field = '".(int)$customization."')
				WHERE 1=1";
                $res=Db::getInstance()->getRow($sql);
                if(!empty($res["id_customization_field"]))
                {
                    $sql2 = "UPDATE "._DB_PREFIX_."customization_field SET is_deleted=1 WHERE id_customization_field=".intval($customization)."";
        ...
        */
		$sql = "UPDATE `"._DB_PREFIX_."product` p SET customizable=(SELECT count(id_customization_field) FROM `"._DB_PREFIX_."customization_field` cf WHERE cf.id_product=p.id_product ".(version_compare(_PS_VERSION_, '1.7.3.0', '>=')?" AND cf.is_deleted='0' ":"")."), 
		...
		{
			$sql = "UPDATE `"._DB_PREFIX_."product_shop` p SET  customizable=(SELECT count(id_customization_field) FROM `"._DB_PREFIX_."customization_field` cf WHERE cf.id_product=p.id_product ".(version_compare(_PS_VERSION_, '1.7.3.0', '>=')?" AND cf.is_deleted='0' ":"")."), 
																													text_fields=(SELECT count(id_customization_field) FROM `"._DB_PREFIX_."customization_field` cf WHERE cf.id_product=p.id_product AND cf.type=1 ".(version_compare(_PS_VERSION_, '1.7.3.0', '>=')?" AND cf.is_deleted='0' ":"")."), 
																													uploadable_files=(SELECT count(id_customization_field) FROM `"._DB_PREFIX_."customization_field` cf WHERE cf.id_product=p.id_product AND cf.type=0 ".(version_compare(_PS_VERSION_, '1.7.3.0', '>=')?" AND cf.is_deleted='0' ":"").")";
			Db::getInstance()->Execute($sql);
		}
