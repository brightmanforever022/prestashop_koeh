...
{
	$sql = "SELECT pa.id_product_attribute, pa.id_product FROM "._DB_PREFIX_."product_attribute pa where pa.id_product_attribute NOT IN (SELECT pas.id_product_attribute FROM "._DB_PREFIX_."product_attribute_shop pas) ORDER BY id_product_attribute ASC LIMIT 1500";
	...
	if(!empty($res) && count($res)>0)
	{
		...
		<script type="text/javascript">
			...
			tbGhostCombi.addButton("selectall", 0, "", "lib/img/application_lightning.png", "lib/img/application_lightning.png");
			tbGhostCombi.setItemToolTip('selectall','<?php echo _l('Select all')?>');
			tbGhostCombi.addButton("delete", 0, "", "lib/img/delete.gif", "lib/img/delete.gif");
			...
			dhxlSCExtCheck.tabbar.cells("table_CAT_COMBI_GHOST_MS").attachStatusBar({text: '<?php echo _l('Count rows:')." ".count($res) ; ?>', height: 25 });
			...
			foreach($post_ids AS $id_product_attribute)
			{
				if (is_numeric($id_product_attribute) && $id_product_attribute)
				{
					...
					if (!$p->hasAttributes())
					{
						...
						$sql="UPDATE "._DB_PREFIX_."product_shop SET cache_default_attribute='0' WHERE id_product='".intval($id_product)."' AND id_shop IN (".pSQL(shopID).") ";
						dbExecuteForeignKeyOff($sql);
					}
					else
					...
					if(1 == $post_for_PS) {
						...
						foreach ($shop_list as $shop_id) {
							...
							$depends_on_stock = StockAvailable::dependsOnStock($id_product, intval($id_shop));
