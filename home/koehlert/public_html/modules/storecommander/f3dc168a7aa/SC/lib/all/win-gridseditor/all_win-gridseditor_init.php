...
?><script type="text/javascript">
	...
	// Colonne des grilles
		...
		dhxlSCGridsEditor.tbGrids.attachEvent("onClick",
			...
			{
				dhxlSCGridsEditor.gridFields.clearAll(true);
				dhxlSCGridsEditor.tbGrids.setListOptionSelected('type','type_combinationmultiproduct');
				dhxlSCGridsEditor.tbGrids.setItemText('type', "<?php echo _l('Catalog:')?> <?php echo _l('Multi-product combinations')?>");
			...
			{
				dhxlSCGridsEditor.gridFields.clearAll(true);
				dhxlSCGridsEditor.tbGrids.setListOptionSelected('type','type_cms');
				dhxlSCGridsEditor.tbGrids.setItemText('type', "<?php echo _l('CMS:')?> <?php echo _l('CMS')?>");
			...
			{
				dhxlSCGridsEditor.gridFields.clearAll(true);
				dhxlSCGridsEditor.tbGrids.setListOptionSelected('type','type_gmapartner');
				dhxlSCGridsEditor.tbGrids.setItemText('type', '<?php echo _l('Addons:',1)?> <?php echo _l('Affiliation program',1)?> - <?php echo _l('Partners',1)?>');
					...
					if (id=='delete')
					{
						if(confirm('<?php echo _l('Are you sure that you want to remove this field?',1)?>'))
						{
							...
							if (ids!=undefined && ids!=null && ids!="")
							{
								...
								$.post("index.php?ajax=1&act=all_win-gridseditor_fields_update&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{"action":"delete","type":type_selected,"grid_select":grid_selected,"ids":ids},function(data){
			...
			dhxlSCGridsEditor.tbEnableFields.attachEvent("onClick",
	                function(id){
	                    if (id=='add')
	                    {
	                        ...
	                        <?php if(SC_GRIDSEDITOR_PRO_INSTALLED) { ?>
