...
{/if}
    ...
    <div class="row">
      <div class="col-md-2">
        <label>{l s='Was not sold for'}</label>
        {html_options name=productFilter_not_sold_time options=$not_sold_time_filter_options selected=$not_sold_time_selected}
      </div>
      <div class="col-md-2">
        <label>{l s='Product older'}</label>
        {html_options name=productFilter_age options=$age_filter_options selected=$age_selected}
      </div>
      <div class="col-md-2">
        <label>{l s='Products in stock'}</label>
        <div>
        <input type="hidden" name="productFilter_in_stock" value="0">
        <input type="checkbox" value="1" name="productFilter_in_stock" {if isset($in_stock_selected) && $in_stock_selected == 1}checked{/if}>
        </div>
      </div>
    </div>
    <hr>
							...
							{else}
								...
								{elseif $params.type == 'select'}
									{if isset($params.filter_key)}
										...
										<select class="filter center" id="{$params.filter_key|replace:'!':''}MultySelect" name="{$list_id}Filter_{$params.filter_key}[]" multiple="multiple"  
											{if isset($params.width)} style="width:{$params.width}px"{/if} >
											{html_options options=$params.list selected=$params.value}
										</select>
                                                                                <script type="text/javascript">
				                                                //<![CDATA[
                                                                                $("#{$params.filter_key|replace:'!':''}MultySelect").multipleSelect({
                                                                                    multiple: true,
                                                                                    {if $params.multipleWidth}multipleWidth: {$params.multipleWidth},{/if}
                                                                                });
                                                                                //]]>
                                                                                </script>
										{else}
					...
					{if $has_actions || $show_filters}
						<th class="actions">
							...
							<span class="pull-right">
								...
								{if $filters_has_value || (isset($plmSelectedSizes) && count($plmSelectedSizes))}
