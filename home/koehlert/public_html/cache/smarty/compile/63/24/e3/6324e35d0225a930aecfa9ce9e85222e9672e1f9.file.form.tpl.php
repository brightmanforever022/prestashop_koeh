<?php /* Smarty version Smarty-3.1.19, created on 2019-08-20 09:03:28
         compiled from "/home/koehlert/public_html/admin971jqkmvw/themes/default/template/controllers/orders/form.tpl" */ ?>
<?php /*%%SmartyHeaderCode:13097844225d5b9b40268600-52906222%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '6324e35d0225a930aecfa9ce9e85222e9672e1f9' => 
    array (
      0 => '/home/koehlert/public_html/admin971jqkmvw/themes/default/template/controllers/orders/form.tpl',
      1 => 1560853270,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '13097844225d5b9b40268600-52906222',
  'function' => 
  array (
  ),
  'variables' => 
  ...
  'unifunc' => 'content_5d5b9b403d28a0_72626164',
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_5d5b9b403d28a0_72626164')) {function content_5d5b9b403d28a0_72626164($_smarty_tpl) {?>
...
",
		...
		$('#id_product').live('change', function(e) {
			e.preventDefault();
			currentProductImageShow($(this).val());
		});
...
').blur();
		...
		$('#customer_cart').on('click', '.cart_product_name', function(e){
			var productName = $(this).text();
			var productRel = $(this).attr('rel');
			var productNameInput = $('<input/>');
			productNameInput.attr({
				type: 'text',
				name: 'cart_product_name['+productRel+']'
			})
			.val(productName);
			$(this).before(productNameInput).hide();
		});
	});
	...
	function currentProductImageShow(id_product){
		var imageUrl = '';
		for(var i = 0; i < productsFound.length; i++){
			if( productsFound[i].id_product == id_product ){
				imageUrl = productsFound[i].image_link;
				break;
			}
		}
		$('#current_product_image').attr('src', imageUrl);
	}
	...
	function resetBind()
	{
        ...
        {
            // show loader icon
            $('#expectedStock').html('<i class="icon-refresh icon-spin"></i>&nbsp;<?php echo smartyTranslate(array('s'=>'Loading','js'=>1),$_smarty_tpl);?>
 ...');
...
');
	...
	function updateCartProducts(products, gifts, id_address_delivery)
	{
		...
		$.each(products, function() {
			...
			cart_content += '<tr>'+
				'<td><img src="'+this.image_link+'" title="'+this.name+'" /></td>';
			
			var productNameEditInput = $('#customer_cart tbody').find('input[name="cart_product_name['+this.id_product+'_'+this.id_product_attribute+']"]');
			if( productNameEditInput.length ){
				cart_content += '<td>'+ '<input type="text" value="'+ productNameEditInput.val() +'" name="cart_product_name['+this.id_product+'_'+this.id_product_attribute+']">' +
					'<span class="cart_product_name hidden" rel="'+this.id_product+'_'+this.id_product_attribute+'">'+this.name+'</span>'+
					'<br />'+this.attributes_small+'</td>';
			}
			else{
				cart_content += '<td><span class="cart_product_name" rel="'+this.id_product+'_'+this.id_product_attribute+'">'+this.name+'</span>'+
					'<br />'+this.attributes_small+'</td>';
			}
			
			cart_content += '<td>'+this.reference+'</td>'+
				'<td><input type="text" rel="'+this.id_product+'_'+this.id_product_attribute+'" class="product_unit_price" value="' + this.numeric_price + '" /></td>'+
				'<td>';
...
';
	...
	function displaySummary(jsonSummary)
	{
		...
		$('#total_items').text( jsonSummary.summary.products.length );
		
		var totalQuantity = 0;
		for( var pi in jsonSummary.summary.products ){
			totalQuantity += jsonSummary.summary.products[pi].quantity;
		}
	...
	{
		if( isNaN(id_cart) || (id_cart <= 0) ){
			alert('Cart not defined');
			return;
		}
		$.ajax({
			type:"POST",
...
",
			...
			success : function(res)
			{
                ...
                $('#submitAddProduct').text('<?php echo smartyTranslate(array('s'=>'Product has been added'),$_smarty_tpl);?>
');
				setTimeout(function() {
	...
	function addProductProcess()
	{
		if (customization_errors) {
			...
			var productIdSelected = parseInt($('#id_product').val());
			var productDataToAdd = {};
			for(var i in productsFound){
				if( parseInt(productsFound[i]['id_product']) == productIdSelected ){
					productDataToAdd = productsFound[i];
				}
			}
			if(productDataToAdd.stock_clearance == '1'){
				if(!confirm(txt_add_product_stock_clearance)){
					return;
				}
			}
			if(productDataToAdd.cstmr_main_excluded == true){
				if(!confirm(productDataToAdd.cstmr_main_excluded_message)){
					return;
				}
				$.ajax({
					type:"POST",
					url: "<?php echo addslashes($_smarty_tpl->tpl_vars['link']->value->getAdminLink('AdminMainexclCustomers'));?>
",
					dataType: "json",
					data : {
						ajax: "1",
						token: "<?php echo $_smarty_tpl->smarty->registered_plugins[Smarty::PLUGIN_FUNCTION]['getAdminToken'][0][0]->getAdminTokenLiteSmarty(array('tab'=>'AdminMainexclCustomers'),$_smarty_tpl);?>
",
						action: "addedToCartLog",
						id_customer: id_customer,
						id_cart: id_cart,
						id_product: productIdSelected
					}
				});
			}
...
">
		...
		<hr/>
<div class="row">
	<div class="col-md-1"><img src="" alt="" id="current_product_image"></div>
		<div id="products_found" class="col-md-11">
		...
		<div class="form-group">
			...
			<div class="col-lg-9">
				<select id="id_order_type" name="id_order_type">
					<?php  $_smarty_tpl->tpl_vars['order_type'] = new Smarty_Variable; $_smarty_tpl->tpl_vars['order_type']->_loop = false;
 $_from = $_smarty_tpl->tpl_vars['order_types']->value; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array');}
foreach ($_from as $_smarty_tpl->tpl_vars['order_type']->key => $_smarty_tpl->tpl_vars['order_type']->value) {
$_smarty_tpl->tpl_vars['order_type']->_loop = true;
?>
						<option value="<?php echo $_smarty_tpl->tpl_vars['order_type']->value['id_order_type'];?>
"><?php echo $_smarty_tpl->tpl_vars['order_type']->value['name'];?>
</option>
					<?php } ?>
				</select>
			</div>
			...
			<div class="row">
				<div class="col-lg-1">
					<div class="data-focus">
						<span><?php echo smartyTranslate(array('s'=>'Total items'),$_smarty_tpl);?>
</span><br/>
						<span id="total_items" class="size_l text-success"></span>
					</div>
				</div>
				<div class="col-lg-1">
					<div class="data-focus">
						<span><?php echo smartyTranslate(array('s'=>'Total quantity'),$_smarty_tpl);?>
</span><br/>
						<span id="total_quantity" class="size_l text-success"></span>
					</div>
				</div>
