...
$(document).ready(function()
{
	...
	$('body.adminorders,body.admincustomers').on('dblclick', '#customerCreditLimitValue', function(){
		$('#customerCreditLimitDialog').modal('show');
		$('#customerCreditLimitDialog .modal-body').html('Loading...');
		$.ajax({
			url: customerCreditLimitController + '&addcustomer_credit_limit_history&ajax=1',
			success: function(response){
				$('#customerCreditLimitDialog .modal-body').html(response);
			}
		});
	});
	$('#customerCreditLimitDialog').on('click', '#customerCreditLimitDialogSave', function(){
		var formData = $('#customer_credit_limit_history_form').serializeArray();
		formData.push({name: 'id_customer', value: id_customer});
		$('#customerCreditLimitDialog .modal-body').html('Loading...');
		$.ajax({
			data: formData,
			method: 'POST',
			url: customerCreditLimitController + '&addcustomer_credit_limit_history&ajax=1&action=add',
			success: function(response){
				...
				if( responseJson !== null ){
					...
					$('#customerCreditLimitDialog').modal('hide');
					$('#customerCreditLimitValue').html(responseJson.credit_limit_formatted);
				}
				else{
					$('#customerCreditLimitDialog .modal-body').html(response);
				}
