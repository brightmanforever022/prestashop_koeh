...
class Msss_client extends Module
{
    ...
    public function install()
    {
        ...
        return parent::install() && 
                //$this->registerHook('actionOrderEdited'),'actionOrderStatusPostUpdate'
        ...
        if ($this->active)
        {
            ...
            foreach($orderDetails as $orderDetail)
            {
                MSSSClientStockUpdater::scheduleStockUpdate($orderDetail['product_supplier_reference'], 
                        -$orderDetail['product_quantity'], $params['order']->id);
            }
            MSSSClientStockUpdater::sendMessagesToServerParallel();
        }
    }
    
    
    public function scheduleStockUpdateById($productId, $combinationId, $deltaQty, $orderId)
    {
        if ($this->active)
        {
            include_once dirname(__FILE__).'/lib/MSSSClientStockUpdater.php';
            // report stock update 
            MSSSClientStockUpdater::scheduleStockUpdateById($productId, $combinationId, $deltaQty, $orderId);
        }
    }
    
    
    public function scheduleClearanceModeChange($sku, $clearance)
    {
        if ($this->active)
        {
            include_once dirname(__FILE__).'/lib/MSSSClientStockUpdater.php';
            // report stock update 
            MSSSClientStockUpdater::scheduleClearanceModeChange($sku, $clearance);
        }
    }
     ...
     */
    public function sendMessagesToServerParallel()
    {
        if ($this->active)
        {
            include_once dirname(__FILE__).'/lib/MSSSClientStockUpdater.php';
