		...
		switch($col){
			...
			case 'image':
				if (file_exists(SC_PS_PATH_REL."img/p/".getImgPath(intval($imgrow['id_product']),intval($imgrow['id_image']),_s('CAT_PROD_GRID_IMAGE_SIZE')))) {
					$return .= "<cell><![CDATA[<img src='".SC_PS_PATH_REL."img/p/".getImgPath(intval($imgrow['id_product']),intval($imgrow['id_image']),_s('CAT_PROD_GRID_IMAGE_SIZE'))."?time=".time()."'/>".(_s('CAT_PROD_IMG_SAVE_FILENAME') && _s('CAT_PROD_IMG_DISPLAY_FILENAME')?'<br/>'.$imgrow['sc_path']:'')."]]></cell>";
				} else {
