		...
		switch($action){
			...
			case 'image_fill_legend_current_lang':
                ...
                if(!(empty($id_product)))
                {
                    ...
                    if(!empty($cache_product_name[$id_product]))
                    {
                        $sql2 = "UPDATE " . _DB_PREFIX_ . "image_lang SET legend='" . pSQL($cache_product_name[$id_product]) . "' WHERE id_image=" . intval($id_image) . " AND id_lang=" . intval($id_lang);
                        Db::getInstance()->Execute($sql2);
                    }
                }
				break;
				...
				if(!(empty($id_product)))
                {
                    ...
                    if(!empty($cache_product_name[$id_product]))
                    {
                        ...
                        foreach ($res as $language)
                        {
                            ...
                            {
                                $sql2 = "UPDATE " . _DB_PREFIX_ . "image_lang SET legend='" . pSQL($cache_product_name[$id_product][$language['id_lang']]) . "' WHERE id_image=" . intval($id_image). " AND id_lang=" . intval($language['id_lang']);
                                Db::getInstance()->Execute($sql2);
                            }
                        }
                    }
			...
			case 'shop':
				...
				if(!empty($id_shop))
				{
					...
					if(!$image->isAssociatedToShop($id_shop) && $value=="1")
					{
						...
						if(version_compare(_PS_VERSION_, '1.6.1', '>='))
                        {
                            if(version_compare(_PS_VERSION_, '1.6.1.3', '>=') && empty($is_cover))
                                $is_cover = "NULL";
                            else
