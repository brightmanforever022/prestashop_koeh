...
if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')) { ?>
	...
	function initCarrier()
	{
		if (needInitCarrier)
		{
			...
			{
                idxDelay=prop_tb._carrierGrid.getColIndexById('delay');
                ...
                {
                    prop_tb._carrierGrid.setRowTextBold(rId);
                    $.post("index.php?ajax=1&act=cat_carrier_update&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{'id_carrier': rId,'delay':nValue},function(data)
                    {
                        if(data == 'OK') {
                            prop_tb._carrierGrid.setRowTextNormal(rId);
                        }
                    });
                }
				return true;
