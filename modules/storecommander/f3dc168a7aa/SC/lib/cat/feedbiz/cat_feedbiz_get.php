	...
	function getFeedBizOptions()
    {
        ...
        switch($view) {
                                ...
                                case 'attribute_name':
                                    if(!empty($id_product_attribute)) {
                                        if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')) {
                                            ...
                                            $detail = array();
                                            $attributes = $prod->getAttributeCombinaisons((int)$lang_id);
                                            foreach($attributes as $attr) {
                                                if($attr['id_product_attribute'] == $id_product_attribute) {
            ...
            case 'grid_feedbiz_amazon_option':
                ...
                $amazon_regions = $module_feedbiz::$cdiscount_regions;
                                ...
                                case 'attribute_name':
                                    ...
                                    if(!empty($id_product_attribute)) {
                                        if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')) {
                                            ...
                                            $detail = array();
                                            $attributes = $prod->getAttributeCombinaisons((int)$id_lang);
                                            foreach($attributes as $attr) {
                                                if($attr['id_product_attribute'] == $id_product_attribute) {
                                                    $detail[] = $attr['group_name'].' : '.$attr['attribute_name'];
                                                }
