...
?>
    ...
    <?php if(KAI9DF4!=1) { ?>
    var opts = [
        ['product_eservices_dixit_addproject', 'obj', '<?php echo _l('Product translation Pro',1)." - "._l('Create a project with the selected products',1)?>', 'lib/img/flag_blue.png'],
        ['product_eservices_dixit_seeproject', 'obj', '<?php echo _l('Product translation Pro',1)." - "._l('See projects',1)?>', 'lib/img/flag_blue.png']
    ];
    cat_grid_tb.addButtonSelect("product_eservices_list", 0, "",opts, "lib/img/ruby.png", "lib/img/ruby.png",false,true);
    cat_grid_tb.setItemToolTip("product_eservices_list","<?php echo _l('e-Services',1)?>");
    <?php } ?>
	if (!isIPAD){
...
?>
			...
			if (id=='exportcsv'){
				displayQuickExportWindow(cat_grid,1);
			}
			if (id=='print'){
				cat_grid.printView();
			}
			if(id=='product_eservices_dixit_addproject')
			{
				loadWindoweServicesProject("dixit");
				
				var name = prompt('<?php echo _l('Name of your new project',1)?>','');
				if(name!=undefined && name!="" && name!=null && name!=0)
				{
					list=cat_grid.getSelectedRowId();
					if(list!="")
					{
						$.post("index.php?ajax=1&act=all_fizz_win-project_update&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{'action': 'add','type': 'dixit', 'name': name, 'items':list},function(data){
							displayProjects();
						});
					}
				}
			}
			if(id=='product_eservices_dixit_seeproject')
			{
				loadWindoweServicesProject("dixit");
			}
			if (id=='setposition'){
...
?>
	...
	var contextMenuXML='<menu absolutePosition="auto" mode="popup" maxItems="8"  globalCss="contextMenu" globalSecondCss="contextMenu" globalTextCss="contextMenuItem">'+
        ...
        <?php }
         if(_r("ACT_CAT_CONTEXTMENU_MASS_UPDATE_PRODUCT")) { ?>
...
function displayProducts(callback)
{
	if (catselection!=undefined && catselection!=null && catselection!="" && catselection!=0)
	{
		...
		cat_grid.load("index.php?ajax=1&act=cat_product_get&tree_mode="+tree_mode+"&productsfrom="+displayProductsFrom+"&idc="+catselection+"&view="+gridView+params_supp+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function(){
