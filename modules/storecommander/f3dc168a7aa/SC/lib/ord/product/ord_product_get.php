 ...
 **/
	...
	function getRowsFromDB(){
		global $id_order,$id_lang,$orderStatus;
		$yesno=array(0=>_l('No'),1=>_l('Yes'));
		$sql = '
			SELECT od.*, pl.name as p_name, '.(version_compare(_PS_VERSION_, '1.5.0.0', '>=') ? 'ps.id_category_default' : 'p.id_category_default').'
			FROM '._DB_PREFIX_.'order_detail od 
                ...
                LEFT JOIN '._DB_PREFIX_.'product_lang pl ON ('.(version_compare(_PS_VERSION_, '1.5.0.0','>=')?"ps.id_product":"p.id_product")." = pl.id_product AND pl.id_lang=".intval($id_lang).(version_compare(_PS_VERSION_, '1.5.0.0', '>=')?" AND pl.id_shop=o.id_shop":"").')
...
//				$actual_quantity_in_stock -= $history['product_quantity_in_stock'];
			...
			$xml.=("<row id='".$history['id_order_detail']."'>");
				...
				$xml.=("<cell><![CDATA[".$history['p_name'].(!empty($combination_detail) ? " ".$combination_detail : '')."]]></cell>");
				$xml.=("<cell>".$history['product_quantity']."</cell>");
				$xml.=("<cell>".$actual_quantity_in_stock."</cell>");
				if (version_compare(_PS_VERSION_, '1.2.0.0', '>='))
					$xml.=("<cell>".($history['product_quantity_in_stock']+$history['product_quantity'])."</cell>");
				if (version_compare(_PS_VERSION_, '1.4.0.0', '>='))
                ...
                if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
                    $xml.=("<cell>".number_format($history['unit_price_tax_excl'], 6, '.', '')."</cell>");
...
<head>
<beforeInit>
<call command="attachHeader"><param><![CDATA[#numeric_filter,#numeric_filter,#numeric_filter,#numeric_filter,#text_filter,#numeric_filter,#numeric_filter<?php if (version_compare(_PS_VERSION_, '1.2.0.0', '>=')){ ?>,#numeric_filter<?php } ?><?php if (version_compare(_PS_VERSION_, '1.4.0.0', '>=')){ ?>,#select_filter<?php } ?><?php if (version_compare(_PS_VERSION_, '1.2.0.0', '>=')){ ?>,#numeric_filter<?php } ?>,#numeric_filter,#numeric_filter<?php if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')){ ?>,#numeric_filter<?php } ?>,#text_filter<?php if (version_compare(_PS_VERSION_, '1.4.0.0', '>=')){ ?>,#text_filter<?php } ?>,#text_filter,#text_filter,#numeric_filter<?php if (version_compare(_PS_VERSION_, '1.5.0.0', '<')){ ?>,#numeric_filter,#numeric_filter<?php } ?>]]></param></call>
