...
/*			$sql="SELECT ".$sqlProduct.$sqlProductLang.",'-' AS position".
		...
		if ($view=='grid_picking' || sc_in_array("id_order_detail", $cols,"ordGet_cols"))
		{
			$sql = "SELECT od.id_order_detail,o.id_order,".( SCAS ?'od.id_warehouse,w.name AS warehousename,':'')."od.product_quantity,od.product_id,od.product_reference,pl.name as product_name,od.product_name as order_detail_product_name,
											...
											".(SCAS?" (SELECT wpl2.location FROM "._DB_PREFIX_."warehouse_product_location wpl2 WHERE wpl2.id_product=od.product_id AND wpl2.id_warehouse=od.id_warehouse LIMIT 1) AS location "
														:" p.location AS location ").(version_compare(_PS_VERSION_, '1.5.0.0', '<') ? ",0 as id_shop_customer" : ",c.id_shop as id_shop_customer");
...
//				$sql.= ",NOT(SELECT COUNT(od2.id_order_detail) FROM "._DB_PREFIX_."order_detail od2 WHERE od2.id_order_detail = od.id_order_detail AND product_quantity_in_stock < product_quantity) as instock, od.product_quantity_in_stock ";
			...
			$sql.= "	LEFT JOIN "._DB_PREFIX_."product_lang pl ON (".(version_compare(_PS_VERSION_, '1.5.0.0', '>=')?"ps.id_product":"p.id_product")." = pl.id_product AND pl.id_lang=".intval($id_lang).(version_compare(_PS_VERSION_, '1.5.0.0', '>=')?" AND pl.id_shop=o.id_shop":"").") ";
			$sql.= "	LEFT JOIN "._DB_PREFIX_."category_lang cl ON (".(version_compare(_PS_VERSION_, '1.5.0.0', '>=')?"ps.id_category_default":"p.id_category_default")." = cl.id_category AND cl.id_lang=".intval($id_lang).(version_compare(_PS_VERSION_, '1.5.0.0', '>=')?" AND cl.id_shop=o.id_shop":"").") ";
			...
			$sql = "SELECT o.id_order,".psql($sqlOrder).(version_compare(_PS_VERSION_, '1.5.0.0', '<') ? ",0 as id_shop_customer" : ",c.id_shop as id_shop_customer");
			if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
			{
