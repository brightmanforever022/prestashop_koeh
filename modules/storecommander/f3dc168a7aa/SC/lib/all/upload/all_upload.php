			    ...
			    if(SCMS) {
			        $sql = 'SELECT id_product,id_product_attribute 
                            ...
                            WHERE id_product_attribute IN (SELECT id_product_attribute 
                                                            ...
                                                            AND id_product_attribute IN (' . pSQL($attr_list) . '))';
                } else {
			        $sql = 'SELECT id_product,id_product_attribute 
			            FROM ' . _DB_PREFIX_ . 'product_attribute
			            AND id_product_attribute IN (' . pSQL($attr_list) . ')';
...
//			d($ids_manufacturer);
	...
	if(!empty($products)) {
        $sql = "UPDATE "._DB_PREFIX_."product SET date_upd = '".pSQL(date("Y-m-d H:i:s"))."' WHERE id_product IN (".pSQL($products).");";
        if(version_compare(_PS_VERSION_, '1.5.0.0', '>=')) {
            $sql .= "UPDATE "._DB_PREFIX_."product_shop SET date_upd = '".pSQL(date("Y-m-d H:i:s"))."' WHERE id_product IN (".pSQL($products).") AND id_shop IN (".pSQL(SCI::getSelectedShopActionList(true)).")";
        }
        Db::getInstance()->Execute($sql);
    }
}
