			...
			if (id=='user_go'){
				...
				if (sel)
				{
					...
					if (tabId.length==1){
						...
						var id_shop = cus_grid.getUserData(tabId[0],'id_shop_customer');
                        connectAsUser("<?php echo SCI::getConfigurationValue('SC_FOLDER_HASH');?>","<?php echo $sc_agent->id_employee; ?>",id_customer,id_shop);
					}else{
...
?>
        ...
        if(cus_grid.getColIndexById("id_address")!=undefined && cus_grid.getColIndexById("id_address")!="")
        {
            idxIdAddress=cus_grid.getColIndexById('id_address');
            var id_address = cus_grid.cells(id,idxIdAddress).getValue();
            new_url = new_url+'&id_address='+id_address;
        }
...
function displayCustomers(callback)
{
	...
	cus_grid.loadXML(loadUrl,function(){
		...
		// Tri indiff�remment de la case ou du caract�re
