...
//console.log("LOAD: "+ui_settings[grid_name]);
	...
	function openSettingsWindow(tools,section,item)
	{
        ...
        if (!dhxWins.isWindow('wCoreSettings'))
        {
            ...
            wCoreSettings.setIcon('lib/img/cog_edit.png','../../../lib/img/cog_edit.png');
	...
	function connectAsUser(folder_hash,employee,id_customer,id_shop)
	{
        $.post('index.php?ajax=1&act=all_loggued-as-user',{'tmp':1,'employee':employee,'id_customer':id_customer,'id_shop':id_shop},function(token){
            var connect_domain = shopUrls[id_shop];
            window.open(connect_domain+"modules/storecommander/"+folder_hash+"/SC/lib/all/all_loggued-as-user.php?token="+token+"&file="+employee+"&SCMS="+SCMS);
        });
    }
