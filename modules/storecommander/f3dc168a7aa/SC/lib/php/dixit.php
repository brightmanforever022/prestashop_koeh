...
function dixit_getTotalNbWords($textes)
{
    ...
    if(!empty($textes))
    {
        ...
        $ret = sc_file_post_contents('http://api.storecommander.com/Dixit/TotalNbWords/', $posts, $headers,true);
        if(!empty($ret))
        {
        ...
        {
            $nb_words += dixit_word_count($text);
        }*/
    }
    return $nb_words;
}

/*function dixit_word_count($string) {
    if (preg_match( '/[\p{Cyrillic}]/u', $string)) {
        return count(preg_split("/\s+/", $string)) - 1;
    }
...
function dixit_getAllPdtTextes($pdts,$sources,$id_lang)
{
    ...
    foreach ($pdts as $id)
    {
        ...
        if(SCMS)
        {
            ...
            $p = new Product($id, false, null, $p->id_shop_default);
        }
        else
            $p = new Product($id);
...
function dixit_action_getQuote($project)
{
    ...
    if(!empty($project["list_items"]) && $project["list_items"]!="-")
    {
        ...
        $price = Dixit::getPrice($textes, $advanced);
    }
    return $price;
}
    ...
    if(!empty($project["list_items"]) && $project["list_items"]!="-")
    {
        ...
        foreach ($pdts as $pdt)
        {
            ...
            if($do)
            {
                if(SCMS)
                {
                    $p = new Product($pdt, true);
                    $p = new Product($pdt, false, null, $p->id_shop_default);
                }
                else
                    $p = new Product($pdt);

                if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
                    $comment = $link->getProductLink($p, null,null,null, $id_lang, $p->id_shop_default);
                else
                    $comment = $link->getProductLink($p, null,null,null, $id_lang);
                ...
                $posts["textes"] = json_encode($textes);
                $posts["amount"] = $amount;
                if(defined("IS_SUBS") && IS_SUBS=="1")
                    $posts["SUBSCRIPTION"] = "1";
                $ret = makeCallToOurApi("Dixit/createTextMulti/", $headers, $posts, true);
        ...
        if(defined("IS_SUBS") && IS_SUBS=="1")
            $posts["SUBSCRIPTION"] = "1";
        $ret = makeCallToOurApi("Fizz/Project/Get/".$project["id_project"],$headers,$posts, true);
...
function dixit_action_checkStatus($project)
{
    ...
    if(in_array($project["status"],array("10","111")))
    {
        ...
        if(defined("IS_SUBS") && IS_SUBS=="1")
            $posts["SUBSCRIPTION"] = "1";
        $ret = makeCallToOurApi("Dixit/checkStatus/", $headers, $posts, true);
...
function dixit_action_importDatas($project)
{
    ...
    if(in_array($project["status"],array("11","12")))
    {
        ...
        foreach ($pdts as $pdt)
        {
            ...
            if(defined("IS_SUBS") && IS_SUBS=="1")
                $posts["SUBSCRIPTION"] = "1";
            $ret = makeCallToOurApi("Dixit/getTranslatedTextes/", $headers, $posts, true);
            if (!empty($ret['code']) && $ret['code'] == "200" && !empty($ret['translated_textes']))
            {
                ...
                foreach ($translated_textes as $txt)
                {
                    ...
                    if(!empty($content))
                    {
                        ...
                        addToHistory('e-services','modification',$type,$pdt,intval($id_lang),_DB_PREFIX_."product_lang",$value,$oldvalue);
                    }
                }
