...
//                    ),
    ...
    {
        if (isset($_REQUEST['slider']) && $_REQUEST['slider'] == 'new')
        {
            $this->context->controller->addCSS(($this->_path) . 'css/slick/slick.css', 'all');
            $this->context->controller->addCSS(($this->_path) . 'css/slick/slick-theme.css', 'all');

            $this->context->controller->addJS('//code.jquery.com/jquery-migrate-1.2.1.min.js');
            $this->context->controller->addJS($this->_path . 'js/slick/slick.min.js');
            if ($hookName == 'displayTopColumn')
            {
                $this->_prepareHook($hookName);
                return $this->display(__FILE__, 'slickslider.tpl', $this->getCacheId($hookName . '_' . $this->name));
            }
        }
        else
        {
