...
class BlockLayered extends Module
{
	...
	public function install()
	{
		...
		&& $this->registerHook('postProcessAttribute') && $this->registerHook('afterDeleteAttribute') && $this->registerHook('afterSaveAttribute') && $this->registerHook('leftColumn'))
		{
			...
			Configuration::updateValue('PS_LAYERED_FILTER_PRICE_ROUNDING', 1);
                        Configuration::updateValue('SORT_SEPARATION_DISTANCE', 7);
	...
	public function indexAttribute($id_product = null)
	{
		...
		Db::getInstance()->execute('
			...
			INNER JOIN '._DB_PREFIX_.'attribute a ON (a.id_attribute = pac.id_attribute)
                        INNER JOIN '._DB_PREFIX_.'attribute_lang al ON (a.id_attribute = al.id_attribute and id_lang='.
                            Configuration::get('PS_LANG_DEFAULT').' and al.name not like \'%muster%\')    
	...
	public function getContent()
	{
		...
		else if (Tools::isSubmit('submitLayeredSettings'))
		{
			...
			Configuration::updateValue('PS_LAYERED_FILTER_CATEGORY_DEPTH', (int)Tools::getValue('ps_layered_filter_category_depth'));
                        Configuration::updateValue('SORT_SEPARATION_DISTANCE', (int)Tools::getValue('ps_layered_filter_sort_sep_distance'));
		...
		else
		{
			$this->context->smarty->assign(array(
				...
				'category_depth' => Configuration::get('PS_LAYERED_FILTER_CATEGORY_DEPTH'),
                                'sort_sep_dist' => intval(Configuration::get('SORT_SEPARATION_DISTANCE')),
	...
	public function displayLimitPostWarning($count)
	{
        ...
        {
            // if we are insearch mode do nothing
            if ($this->searchMode)
            {
                return $this->getSelectedFiltersFromRequest();
            }
            
            $emptyFilters = array('category' => array(), 'manufacturer' => array(), 'quantity' => array(), 'condition' => array(),
				'cat_colors'=>array(), 'id_attribute_group'=>array());
            // determine category we are in
            ...
            // if it is reset
            if (!empty($_REQUEST['blreset']))
            {
                $filtersFromSession = unserialize(Context::getContext()->cookie->bLayeredFilters);
                if (is_array($filtersFromSession))
                {
                    unset($filtersFromSession[$id_parent]);
                }
                Context::getContext()->cookie->bLayeredFilters = serialize($filtersFromSession);
                return $emptyFilters;
            }
            
            $filtersFromRequest = $this->getSelectedFiltersFromRequest();
            
            // if it is usual not ajax call 
            if (strpos($_SERVER['SCRIPT_FILENAME'], 'blocklayered-ajax.php') === false)
            {
                // check filters in url
                if ($filtersFromRequest == $emptyFilters)
                {
                    // try restore filters from session
                    $filtersFromSession = unserialize(Context::getContext()->cookie->bLayeredFilters);
                    if (is_array($filtersFromSession) && isset($filtersFromSession[$id_parent]))
                    {
                        return $filtersFromSession[$id_parent];
                    }
                }
            }
            else
            {
                // we just record filters
                $filtersFromSession = unserialize(Context::getContext()->cookie->bLayeredFilters);
                if (is_array($filtersFromSession))
                {
                    $filtersFromSession[$id_parent] = $filtersFromRequest;
                }
                else
                {
                    $filtersFromSession = [$id_parent => $filtersFromRequest];
            ...
            if (!empty($_REQUEST['blreset']))
            {
                Context::getContext()->cookie->bLayeredFilters = false;
                return $emptyFilters;
            }
            
            
            
            // use/load cached filters only in case if it not ajax request
            if ($filtersFromRequest == $emptyFilters && strpos($_SERVER['SCRIPT_FILENAME'], 'blocklayered-ajax.php') === false)
            {
                // no filters were passed in request, look in session
                if (!empty(Context::getContext()->cookie->bLayeredFilters))
                {
                    $filtersFromRequest = unserialize(Context::getContext()->cookie->bLayeredFilters)[$id_parent];
                }
            }
            else
            {
                // save filters in session for future use
                Context::getContext()->cookie->bLayeredFilters = serialize($filtersFromRequest);
            }
            
            return $filtersFromRequest;*/
        }
        
	private function getSelectedFiltersFromRequest()
	{
		if (!$this->searchMode)
		{
			$home_category = Configuration::get('PS_HOME_CATEGORY');
	...
	public function getProductByFilters($selected_filters = array())
	{
		...
		foreach ($selected_filters as $key => $filter_values)
		{
			...
			switch ($key)
			{
				...
				case 'quantity':
					...
					foreach ($selected_filters as $keyv => $filter_valuesv) {
						preg_match('/^(.*[^_0-9])/', $keyv, $ress);
						$keyv = $ress[1];

						if($keyv == 'id_attribute_group') {
							foreach ($filter_valuesv as $filter_valuev)
							{
								$filter_value_arrayss = explode('_', $filter_valuev);
								if (!in_array($filter_value_arrayss[1], $attr_arr))
								$attr_arr[] = (int)$filter_value_arrayss[1];
							}
						}
					}

					if(!empty($attr_arr)) {
						$or_statement = implode(',', $attr_arr);

						$query_filters_where .= ' AND p.id_product IN (SELECT pa.`id_product`
								FROM `'._DB_PREFIX_.'product_attribute_combination` pac
								LEFT JOIN `'._DB_PREFIX_.'product_attribute` pa
								ON (pa.`id_product_attribute` = pac.`id_product_attribute`)
								LEFT JOIN `'._DB_PREFIX_.'attribute_lang` pas
								ON (pas.`id_attribute` = pac.`id_attribute`)
								LEFT JOIN `'._DB_PREFIX_.'stock_available` past
								ON (pa.`id_product_attribute` = past.`id_product_attribute`)
								WHERE pas.id_lang=1 AND pas.id_attribute IN('.$or_statement.') AND past.quantity > 0) ';
		...
		$sql = 'ALTER TABLE '._DB_PREFIX_.'cat_filter_restriction ADD PRIMARY KEY (id_product), ADD KEY (position, id_product) USING BTREE';
                Db::getInstance()->execute($sql, false);
                //echo $sql;
		...
		else
		{
			...
			if (version_compare(_PS_VERSION_, '1.6.1', '>=') === true)
			{
					...
					stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity'.(Combination::isFeatureActive() ? ', product_attribute_shop.minimal_quantity AS product_attribute_minimal_quantity' : '');
                            ...
                            $sqlTail = ' FROM '._DB_PREFIX_.'cat_filter_restriction cp
				...
				'.Product::sqlStock('p', 0);
                            ...
                            $orderBy = Tools::getProductsOrder('by', Tools::getValue('orderby'), true, $id_parent);
                            if($orderBy=='best_seller')
                            {
                                $sql .= ', ps.quantity as best_seller';
                                $sqlTail .= ' left join '._DB_PREFIX_.'product_sale ps on p.id_product=ps.id_product ';
                            }
                            $sqlTail .= 'WHERE '.$alias_where.'.`active` = 1 AND '.$alias_where.'.`visibility` IN ("both", "catalog") ORDER BY ';
                            if (strpos($orderBy, 'top_seller')!==FALSE)
                            {
                                // special actions related with top seller sort order
                                // if result in cache, get it from cache
                                if (1 || !TopSellerSortTools::checkCache($id_parent, $context, $selected_filters))
                                {
                                    // transfer sql without select part
                                    TopSellerSortTools::putQueryInCache($sqlTail, $parent, $context, $selected_filters, $n);
                                }
                                $this->products = TopSellerSortTools::getProducts($context, $this->page, $n);
                            }
                            else
                            {
                                // run query as usual
                                $orderBy .= ' '.Tools::getProductsOrder('way', Tools::getValue('orderway')).' , cp.id_product';
                                $this->products = Db::getInstance()->executeS($sql.$sqlTail.$orderBy.
	...
	public function ajaxCall()
	{
		...
		$vars = array(
			...
			'pagination' => $this->context->smarty->fetch(_PS_THEME_DIR_.'pagination-category.tpl'),
