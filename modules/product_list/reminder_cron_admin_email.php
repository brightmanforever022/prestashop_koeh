...
function generateReminders1()
{
    ...
    $invoices = Db::getInstance()->executeS('select id_order_invoice, sum_to_pay, number, c.firstname, c.lastname, c.address1, c.company, ' .
            ...
            'o.id_currency, oi.template_id, sum_paid from ' . _DB_PREFIX_ .
            ...
            'INNER JOIN `'._DB_PREFIX_.'ba_prestashop_invoice` bai 
                ON oi.template_id = bai.id AND bai.payment_type != '. (BaOrderInvoice::PAYMENT_TYPE_CREDIT_SLIP) .
...
function generateReminders2()
{
    ...
    $invoices = Db::getInstance()->executeS('select id_order_invoice, sum_to_pay, number, c.firstname, c.lastname, c.address1, c.company, ' .
        ...
        oi.number>0 
        and oi.paid=0 
        and oi.reminder_state='. OrderInvoice::Reminder1Sent. ' 
        and reminder_date <=\''. date('Y-m-d', time()-Configuration::get('dpReminder2Days')*24*3600).'\'
    ...
    if (!empty($invoices))
    {
        ...
        foreach ($invoices as $invoice)
        {
            $invoiceIds []= $invoice['id_order_invoice'];
            if( $reminder2Fee ){
                $invoice['reminder_fee'] = $reminder2Fee;
            }
...
function generateReminders3()
{
    ...
    $invoices = Db::getInstance()->executeS('select id_order_invoice, sum_to_pay, number, c.firstname, c.lastname, c.address1, c.company, ' .
            'c.address2, c.city, c.postcode, o.id_lang, oi.date_add as invoice_date, gl.name as salutation, cl.name as countryName, '.
            'o.id_currency, oi.template_id, oi.reminder_fee, sum_paid from ' . _DB_PREFIX_ .
            'order_invoice oi left join ' . _DB_PREFIX_ . 'orders o on o.id_order=oi.id_order left join ' . _DB_PREFIX_ . 'customer c ' .
            'on c.id_customer=o.id_customer left join ' . _DB_PREFIX_ . 'gender_lang gl on c.id_gender=gl.id_gender and gl.id_lang=' .
            'o.id_lang left join ' . _DB_PREFIX_ . 'address a on o.id_address_invoice=a.id_address left join ' .
            _DB_PREFIX_ . 'country_lang cl on cl.id_country=a.id_country and cl.id_lang=o.id_lang '.
        ...
        oi.number>0 
        and oi.paid=0 
        and oi.reminder_state='.OrderInvoice::Reminder2Sent.' 
        and reminder_date <=\''. date('Y-m-d', time()-Configuration::get('dpReminder3Days')*24*3600).'\'
        and due_date <= \''. date('Y-m-d') .'\'
    ');
...
function sendRemindersToAdmin()
{
    ...
    $invoices = Db::getInstance()->executeS('
        select id_order_invoice, o.id_lang, oi.reminder_state, admin_email_sent 
        from '. _DB_PREFIX_ .'order_invoice oi 
        left join '._DB_PREFIX_.'orders o on o.id_order=oi.id_order
        INNER JOIN `'._DB_PREFIX_.'ba_prestashop_invoice` bai 
            ON oi.template_id = bai.id AND bai.payment_type != '. (BaOrderInvoice::PAYMENT_TYPE_CREDIT_SLIP) .'
        where paid=0 and reminder_state>admin_email_sent '.
            'and reminder_state not in('.OrderInvoice::ReminderInkasso.','.OrderInvoice::ReminderInkasSccs.','.
            OrderInvoice::ReminderCredit.')');
