...
class OrderEdit extends Module
{
    ...
    */
    ...
    private static function fillOrderDetail(OrderDetail &$detail, stdClass $std_rq, array $product, $shop)
    {
        ...
        $detail->id_tax_rules_group = $product['id_tax_rules_group'];
        ...
        if ($detail->id_tax_rules_group == 0) {
            $pri = $product['price'];
    ...
    {
        if (!$detail->save(true, false, false) && !Validate::isLoadedObject($detail)) {
            if (!$backup_detail) {
                self::setAjaxError(
    ...
    public function executeSaveOrder(stdClass $std_rq)
    {
        ...
        $orderDetails = Db::getInstance()->executeS('select product_id, product_attribute_id, id_order_detail, product_quantity, '.
                'product_quantity_refunded, product_quantity_return, shipped '.
        ...
        foreach ($products as &$pr) {
            ...
            $pr['id_tax_rules_group'] = intval($pr['productTaxEdit']);
                ...
                if (!$skip) {
                    if ($cart_result != OrderEdit::PRODUCT_IN_CART_DELETED) {
                        ...
                        if ($detail_errors && count($detail_errors)) {
                            ...
                            if ($detail = $this->saveOrderDetail($detail, $detail_backup, $product['id_tax_rules_group'], $detail_backup_id_tax)) {
        ...
        $std_rq->order->updateOrderDetailTax();

        // check if we need to send notification to server about product qty update
        // create new order details map
        $orderDetails = Db::getInstance()->executeS('select product_id, product_attribute_id, id_order_detail, product_quantity, '.
                'product_quantity_refunded, product_quantity_return, shipped from '._DB_PREFIX_.'order_detail where id_order='.$std_rq->order->id);
        ...
        foreach($orderDetailsMapBefore as $key=>$beforeOd)
        {
            ...
            {
                if($beforeOd['product_quantity']!=$orderDetailsMapAfter[$key]['product_quantity'])
                {
                    // quantity updated
    ...
    private function notifyCustomerByMail(Order $order)
    {
        ...
        Product::initPricesComputation($customer->id);
    ...
    public function executeAddProduct(stdClass $std_rq)
    {
        ...
        if ($other_product_details && count($other_product_details)) {
            ...
            if( $other_product_details['id_tax_rules_group'] > 0 ){
                $taxRulesGroup = new TaxRulesGroup($other_product_details['id_tax_rules_group']);
                $other_product_details['tax_rule_group_name'] = $taxRulesGroup->name;
            }
        ...
        $std_rq->context->smarty->assign(array(
            ...
            'can_edit' => true,
            'unsaved' => true,
            'taxes' => TaxRulesGroup::getTaxRulesGroups()//Tax::getTaxes($std_rq->context->language->id, true)
        ));
