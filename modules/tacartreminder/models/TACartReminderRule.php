...
class TACartReminderRule extends ObjectModel
{
     ...
     */
    public static $rel_condition_typevalue = array(
        'cart_product' => 'list',
    ...
    public static $map_compare = array(
        '=' => 'isEqual',
        '<' => 'isLessThan',
        '<=' => 'isLessOrEqualThan',
        '>' => 'isGreaterThan',
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'cart_rule_nbday_validity' => array(
                'type' => self::TYPE_INT,
    ...
    public static function getRules(
        ...
        if ($available_now) {
            $sql_filter .= ' AND (date_from IS NULL OR date_from = \'\'
            OR date_from = \'0000-00-00 00:00:00\'
            OR date_from <= \''.date('Y-m-d H:i:s').'\' )
            AND (date_to IS NULL OR date_to   = \'\' OR date_to = \'0000-00-00 00:00:00\'
    ...
    public function getGroupConditions()
    {
        ...
        foreach ($result as $row) {
            if (!isset($groups[$row['id_groupcondition']])) {
                $groups[$row['id_groupcondition']] = array(
    ...
    public function getConditions($id_groupcondition)
    {
        ...
        foreach ($results as $row) {
            if (!isset($conditions[$row['id_condition']])) {
                $conditions[$row['id_condition']] = array(
                    'type' => $row['type'],
                    'values' => array(),
                    'typevalue' => '',
                    'value' => '',
    ...
    public function delete()
    {
        ...
        foreach ($journal_rows as $journal_row) {
            ...
            $mess->message = (string)sprintf(
                $this->l('The rule id %1$s hast been deleted, the reminder is canceled.'),
                (int)$this->id
            );
            $mess->is_system = true;
            $mess->add();
    ...
    public static function getApplicableRule(
        ...
        $rules = self::getRules(true, null, null, array(), true, (int)$cart->id_shop);
        $checkrules = array();
        $module_instance = null;
        if ($return_jc) {
            $module_instance = new TACartReminder();
            $checkrules['common_condition'] = array();
            $checkrules['rules'] = array();
