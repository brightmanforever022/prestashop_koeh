 ...
 * + PS version: 1.5,1.6
 *
 * TACartReminderRuleMatchCache class, TACartReminderRuleMatchCache.php
 * Only work if necessary!
 * Deduction rule cache permit to no check a cart : if is already checked & cart not update
 * Standard cache(Memcache, Xcache, APC, ..) is not use because memory used is important after many test persistence
 * cache is better.
 * Performance audit
 * @category models
...
class TACartReminderRuleMatchCache extends ObjectModel
{
    ...
    public $date_check;
    /**
     * @var string $result json_result persistence(used encode to set or decode to get)
     */
    public $result;
    ...
    public static function get($id_cart, $return_jc, $cart_date_upd = null)
    {
        ...
        if ($cache_row) {
            ...
            foreach ($cache_row as $key => $value) {
                if (array_key_exists($key, $cache_rule)) {
