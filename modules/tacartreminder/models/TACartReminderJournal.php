...
class TACartReminderJournal extends ObjectModel
{
     ...
     *
     * @return boolean false if not|array reminder
     */
    ...
    public static function performReminder(
        ...
        $row = Db::getInstance()->getRow(
            'SELECT r.position as reminder_position, r.id_mail_template,
        ...
        if ($row) {
            ...
            if ((int)$row['reminder_position'] > 1) {
                ...
                if (!$reminderbeforeislaunch || !((int)$reminderbeforeislaunch['exist'] > 0)) {
                    throw new PrestaShopException(
                        sprintf(
                            Tools::displayError('Cannot launch reminder position %1$s before reminder position %2$s'),
                            (int)$row['reminder_position'],
                            ((int)$row['reminder_position'] - 1)
                        )
                    );
                }
            }
                    ...
                    if (Validate::isLoadedObject($cart_rule) && $cart_rule->id) {
                        ...
                        $cart_rule->date_to = date(
                            ...
                            mktime(
                                ...
                                date('s'),
                                date('m'),
                                date('d') + (int)$row['cart_rule_nbday_validity'],
                                date('Y')
                            )
                        );
                        ...
                        throw new PrestaShopException(
                            sprintf(
                                Tools::displayError('Can\'t load CartRule object, cart rule id %s is there?'),
                                $row['id_cart_rule']
                            )
                        );
                    }
            ...
            if ($manual_process && $type_perform == 'INIT') {
                ...
                throw new PrestaShopException(
                    sprintf(
                        Tools::displayError('Can\'t load MailTemplate object, the mail template id %s is there?'),
                        $row['id_mail_template']
                    )
                );
            }
            ...
            throw new PrestaShopException(
                sprintf(Tools::displayError('No reminder found cart:%1$s and reminder:%2$s '), $cart->id, $id_reminder)
            );
        }
    }
