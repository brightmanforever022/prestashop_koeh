...
class TACartReminderTools
{
    ...
    public static function send(
        ...
        try {
            ...
            $shop_base_url = 'http://' . Context::getContext()->shop->domain . Context::getContext()->shop->getBaseURI();
    ...
    public static function getCartRecoverUrl($id_reminder, $cart, $context = null, $step = 3, $cart_rule_add = 0)
    {
        ...
        $order_process = version_compare(_PS_VERSION_, '1.7.0.0', '<') && Configuration::get('PS_ORDER_PROCESS_TYPE') ?
        'order-opc' : 'order';
        if ($step > 1) {
            $url = $context->link->getPageLink(
                ...
                true,
                (int)$cart->id_lang,
                'step='.$step.'&recover_cart='.(int)$cart->id.'&tacartreminder_clk=1&id_reminder='.
                (int)$id_reminder.'&cadd='.$cart_rule_add.'&token_cart='.md5(
            ...
            $url = $context->link->getPageLink(
                ...
                true,
                (int)$cart->id_lang,
                'recover_cart='.(int)$cart->id.'&tacartreminder_clk=1&id_reminder='.
                (int)$id_reminder.'&cadd='.$cart_rule_add.'&token_cart='.md5(
                    _COOKIE_KEY_.'recover_cart_'.(int)$cart->id
                ),
                null,
        ...
        if (Validate::isLoadedObject($context->cart) && $context->cart->id) {
            ...
            if ($type_render == 'html') {
                return $context->smarty->fetch(
                    dirname(__FILE__).'/../views/templates/admin/mail_template/shopping-cart.tpl'
                );
            } else {
                return $context->smarty->fetch(
                    dirname(__FILE__).'/../views/templates/admin/mail_template/shopping-cart-txt.tpl'
                );
            }
        }
    ...
    public static function renderMail(
        ...
        $shop_base_url = 'http://'.$context->shop->domain.$context->shop->getBaseURI();
